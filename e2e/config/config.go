...
type Config struct {
	...
	MuseChainID             string                  `yaml:"muse_chain_id"`
}
...
type AdditionalAccounts struct {
	...
	UserLegacyMuse        Account `yaml:"user_legacy_muse"`
	UserLegacyMEVMMP      Account `yaml:"user_legacy_mevm_mp"`
	...
	UserMigration         Account `yaml:"user_migration"` // used for TSS migration, TODO: rename (https://github.com/RWAs-labs/muse/issues/2780)
...
type ObserverRelayerAccounts struct {
	// RelayerAccounts contains two relayer accounts used by museclient0 and museclient1
	RelayerAccounts [2]Account `yaml:"relayer_accounts"`
}

// RPCs contains the configuration for the RPC endpoints
type RPCs struct {
	Mevm              string     `yaml:"mevm"`
	EVM               string     `yaml:"evm"`
	...
	MuseCoreGRPC      string     `yaml:"musecore_grpc"`
	MuseCoreRPC       string     `yaml:"musecore_rpc"`
	MuseclientMetrics string     `yaml:"museclient_metrics"`
}
...
type EVM struct {
	MuseEthAddr      DoubleQuotedString `yaml:"muse_eth"`
...
// MEVM contains the addresses of predeployed contracts on the mEVM chain
type MEVM struct {
	SystemContractAddr DoubleQuotedString `yaml:"system_contract"`
	ETHMRC20Addr       DoubleQuotedString `yaml:"eth_mrc20"`
	ERC20MRC20Addr     DoubleQuotedString `yaml:"erc20_mrc20"`
	BTCMRC20Addr       DoubleQuotedString `yaml:"btc_mrc20"`
	SOLMRC20Addr       DoubleQuotedString `yaml:"sol_mrc20"`
	SPLMRC20Addr       DoubleQuotedString `yaml:"spl_mrc20"`
	TONMRC20Addr       DoubleQuotedString `yaml:"ton_mrc20"`
	SUIMRC20Addr       DoubleQuotedString `yaml:"sui_mrc20"`
	SuiTokenMRC20Addr  DoubleQuotedString `yaml:"sui_token_mrc20"`
	UniswapFactoryAddr DoubleQuotedString `yaml:"uniswap_factory"`
	UniswapRouterAddr  DoubleQuotedString `yaml:"uniswap_router"`
	ConnectorMEVMAddr  DoubleQuotedString `yaml:"connector_mevm"`
	WMuseAddr          DoubleQuotedString `yaml:"wmuse"`
	MEVMSwapAppAddr    DoubleQuotedString `yaml:"mevm_swap_app"`
...
func DefaultConfig() Config {
	return Config{
		RPCs: RPCs{
			...
			MuseCoreGRPC: "musecore0:9090",
			MuseCoreRPC:  "http://musecore0:26657",
			Solana:       "http://solana:8899",
			TON:          "http://ton:8000/lite-client.json",
		},
		MuseChainID: "athens_101-1",
		Contracts: Contracts{
			EVM: EVM{
...
func (c *Config) GenerateKeys() error {
	...
	c.AdditionalAccounts.UserLegacyMuse, err = generateAccount()
	if err != nil {
		return err
	}
	c.AdditionalAccounts.UserLegacyMEVMMP, err = generateAccount()
	if err != nil {
		return err
	}
...
func (a Account) Validate() error {
	...
	bech32PrivateKeyAddress, err := bech32.ConvertAndEncode("muse", privateKeyAddress.Bytes())
	if err != nil {
...
func generateAccount() (Account, error) {
	...
	bech32Address, err := bech32.ConvertAndEncode("muse", evmAddress.Bytes())
	if err != nil {
