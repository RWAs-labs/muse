...
func MinimumVersionCheck(testVersion, musecoredVersion string) bool {
	// If major version is "v0", return true regardless of comparison
	if semver.Major(musecoredVersion) == "v0" {
		return true
	}

	// Otherwise, return true if musecoredVersion >= testVersion
	return semver.Compare(musecoredVersion, testVersion) >= 0
}
...
func FetchNodePubkey(host string) (string, error) {
	...
	cmd := exec.Command("ssh", "-q", fmt.Sprintf("root@%s", host), "musecored tendermint show-validator")
...
func FetchHotkeyAddress(host string) (parsers.ObserverInfoReader, error) {
	// #nosec G204 validation of host should be done by the caller
	cmd := exec.Command("ssh", "-q", fmt.Sprintf("root@%s", host), "cat ~/.musecored/os.json")
	var out bytes.Buffer
	cmd.Stdout = &out
	var stderr bytes.Buffer
	cmd.Stderr = &stderr
