...
// MRC20Withdrawal prints a MRC20Withdrawal event
func (l *Logger) MRC20Withdrawal(
	contract interface {
		ParseWithdrawal(ethtypes.Log) (*mrc20.MRC20Withdrawal, error)
	},
	...
	for _, log := range receipt.Logs {
		...
		l.Info(
			" %s MRC20Withdrawal: from %s, to %x, value %d, gasfee %d",
			name,
			event.From.Hex(),
			event.To,
			event.Value,
