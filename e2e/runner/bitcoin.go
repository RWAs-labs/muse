...
import (
	...
	musebtc "github.com/RWAs-labs/muse/museclient/chains/bitcoin/common"
	btcobserver "github.com/RWAs-labs/muse/museclient/chains/bitcoin/observer"
	"github.com/RWAs-labs/muse/museclient/chains/bitcoin/signer"
...
// DepositBTCWithExactAmount deposits exact 'amount' of BTC to receiver MEVM address
// It automatically adds the depositor fee so that the receiver gets the exact 'amount' in MuseChain
func (r *E2ERunner) DepositBTCWithExactAmount(amount float64, memo *memo.InboundMemo) *chainhash.Hash {
	amount += musebtc.DefaultDepositorFee
...
func (r *E2ERunner) DepositBTCWithAmount(amount float64, memo *memo.InboundMemo) *chainhash.Hash {
	...
	// deposit BTC into MEVM
	if memo != nil {
		r.Logger.Info("⏳ depositing BTC into MEVM with standard memo (amount: %.4f)", amount)
		...
		r.Logger.Info("⏳ depositing BTC into MEVM with legacy memo (amount: %.4f)", amount)
...
// DepositBTC deposits BTC from the Bitcoin node wallet into MEVM address.
func (r *E2ERunner) DepositBTC(receiver common.Address) {
	r.Logger.Print("⏳ depositing BTC into MEVM")
	startTime := time.Now()
	defer func() {
		r.Logger.Print("✅ BTC deposited in %s", time.Since(startTime))
	}()

	r.Logger.Info("Now depositing BTC to MEVM address...")

	// send initial BTC to the tester MEVM address
	amount := 1.15 + musebtc.DefaultDepositorFee
	txHash, err := r.SendToTSSWithMemo(amount, receiver.Bytes())
	require.NoError(r, err)

	r.Logger.Info("testing if the deposit into BTC MRC20 is successful...")
	...
	balance, err := r.BTCMRC20.BalanceOf(&bind.CallOpts{}, r.EVMAddress())
	require.NoError(r, err)
	require.Equal(r, 1, balance.Sign(), "balance should be positive")
}

// WithdrawBTC is a helper function to call 'withdraw' on BTCMRC20 contract with optional 'approve'
func (r *E2ERunner) WithdrawBTC(
	...
	_, gasFee, err := r.BTCMRC20.WithdrawGasFee(&bind.CallOpts{})
	require.NoError(r, err)
	minimumAmount := new(big.Int).Add(amount, gasFee)
	currentBalance, err := r.BTCMRC20.BalanceOf(&bind.CallOpts{}, r.MEVMAuth.From)
	require.NoError(r, err)
	require.Greater(
		r,
	...
	if approve {
		r.ApproveBTCMRC20(r.GatewayMEVMAddr)
	}

	// withdraw 'amount' of BTC through MEVM gateway
...
// WithdrawBTCAndWaitCCTX withdraws BTC from MRC20 contract and waits for the CCTX to be finalized
func (r *E2ERunner) WithdrawBTCAndWaitCCTX(
	...
	// approve and withdraw on MRC20 contract
...
func (r *E2ERunner) sendToAddrWithMemo(
	...
	amountInt, err := musebtc.GetSatoshis(amount)
	require.NoError(r, err)
...
func (r *E2ERunner) InscribeToTSSWithMemo(
	...
	commitAmount, err := musebtc.GetSatoshis(amount)
	require.NoError(r, err)
...
func (r *E2ERunner) QueryOutboundReceiverAndAmount(txid string) (string, int64) {
	...
	receiver, err := musebtc.DecodeScriptP2WPKH(hex.EncodeToString(pkScript), r.BitcoinParams)
	require.NoError(r, err)
