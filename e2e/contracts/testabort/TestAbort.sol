...
contract TestAbort {
    ...
    function onAbort(AbortContext calldata abortContext) external {
        ...
        if (isWithdrawMessage(string(abortContext.revertMessage))) {
            (address feeToken, uint256 feeAmount) = IMRC20(abortContext.asset).withdrawGasFee();
            require(feeToken == abortContext.asset, "mrc20 is not gas token");
            ...
            IMRC20(abortContext.asset).approve(msg.sender, abortContext.amount);
