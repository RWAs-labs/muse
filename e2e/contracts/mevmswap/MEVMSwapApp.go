// Code generated - DO NOT EDIT.
// This file is a generated binding and any manual changes will be lost.

package mevmswap

import (
	"errors"
	"math/big"
	"strings"

	ethereum "github.com/ethereum/go-ethereum"
	"github.com/ethereum/go-ethereum/accounts/abi"
	"github.com/ethereum/go-ethereum/accounts/abi/bind"
	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/core/types"
	"github.com/ethereum/go-ethereum/event"
)

// Reference imports to suppress errors if they are not otherwise used.
var (
	_ = errors.New
	_ = big.NewInt
	_ = strings.NewReader
	_ = ethereum.NotFound
	_ = bind.Bind
	_ = common.Big1
	_ = types.BloomLookup
	_ = event.NewSubscription
	_ = abi.ConvertType
)

// Context is an auto generated low-level Go binding around an user-defined struct.
type Context struct {
	Origin  []byte
	Sender  common.Address
	ChainID *big.Int
}

// MEVMSwapAppMetaData contains all meta data concerning the MEVMSwapApp contract.
var MEVMSwapAppMetaData = &bind.MetaData{
	ABI: "[{\"inputs\":[{\"internalType\":\"address\",\"name\":\"router02_\",\"type\":\"address\"},{\"internalType\":\"address\",\"name\":\"systemContract_\",\"type\":\"address\"}],\"stateMutability\":\"nonpayable\",\"type\":\"constructor\"},{\"inputs\":[],\"name\":\"InvalidSender\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"LowAmount\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"bytes\",\"name\":\"data\",\"type\":\"bytes\"}],\"name\":\"decodeMemo\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"},{\"internalType\":\"bytes\",\"name\":\"\",\"type\":\"bytes\"}],\"stateMutability\":\"pure\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"targetMRC20\",\"type\":\"address\"},{\"internalType\":\"bytes\",\"name\":\"recipient\",\"type\":\"bytes\"}],\"name\":\"encodeMemo\",\"outputs\":[{\"internalType\":\"bytes\",\"name\":\"\",\"type\":\"bytes\"}],\"stateMutability\":\"pure\",\"type\":\"function\"},{\"inputs\":[{\"components\":[{\"internalType\":\"bytes\",\"name\":\"origin\",\"type\":\"bytes\"},{\"internalType\":\"address\",\"name\":\"sender\",\"type\":\"address\"},{\"internalType\":\"uint256\",\"name\":\"chainID\",\"type\":\"uint256\"}],\"internalType\":\"structContext\",\"name\":\"\",\"type\":\"tuple\"},{\"internalType\":\"address\",\"name\":\"mrc20\",\"type\":\"address\"},{\"internalType\":\"uint256\",\"name\":\"amount\",\"type\":\"uint256\"},{\"internalType\":\"bytes\",\"name\":\"message\",\"type\":\"bytes\"}],\"name\":\"onCall\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"components\":[{\"internalType\":\"bytes\",\"name\":\"origin\",\"type\":\"bytes\"},{\"internalType\":\"address\",\"name\":\"sender\",\"type\":\"address\"},{\"internalType\":\"uint256\",\"name\":\"chainID\",\"type\":\"uint256\"}],\"internalType\":\"structContext\",\"name\":\"\",\"type\":\"tuple\"},{\"internalType\":\"address\",\"name\":\"mrc20\",\"type\":\"address\"},{\"internalType\":\"uint256\",\"name\":\"amount\",\"type\":\"uint256\"},{\"internalType\":\"bytes\",\"name\":\"message\",\"type\":\"bytes\"}],\"name\":\"onCrossChainCall\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"router02\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"systemContract\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"}]",
	Bin: "",
}

// MEVMSwapAppABI is the input ABI used to generate the binding from.
// Deprecated: Use MEVMSwapAppMetaData.ABI instead.
var MEVMSwapAppABI = MEVMSwapAppMetaData.ABI

// MEVMSwapAppBin is the compiled bytecode used for deploying new contracts.
// Deprecated: Use MEVMSwapAppMetaData.Bin instead.
var MEVMSwapAppBin = MEVMSwapAppMetaData.Bin

// DeployMEVMSwapApp deploys a new Ethereum contract, binding an instance of MEVMSwapApp to it.
func DeployMEVMSwapApp(auth *bind.TransactOpts, backend bind.ContractBackend, router02_ common.Address, systemContract_ common.Address) (common.Address, *types.Transaction, *MEVMSwapApp, error) {
	parsed, err := MEVMSwapAppMetaData.GetAbi()
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	if parsed == nil {
		return common.Address{}, nil, nil, errors.New("GetABI returned nil")
	}

	address, tx, contract, err := bind.DeployContract(auth, *parsed, common.FromHex(MEVMSwapAppBin), backend, router02_, systemContract_)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	return address, tx, &MEVMSwapApp{MEVMSwapAppCaller: MEVMSwapAppCaller{contract: contract}, MEVMSwapAppTransactor: MEVMSwapAppTransactor{contract: contract}, MEVMSwapAppFilterer: MEVMSwapAppFilterer{contract: contract}}, nil
}

// MEVMSwapApp is an auto generated Go binding around an Ethereum contract.
type MEVMSwapApp struct {
	MEVMSwapAppCaller     // Read-only binding to the contract
	MEVMSwapAppTransactor // Write-only binding to the contract
	MEVMSwapAppFilterer   // Log filterer for contract events
}

// MEVMSwapAppCaller is an auto generated read-only Go binding around an Ethereum contract.
type MEVMSwapAppCaller struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// MEVMSwapAppTransactor is an auto generated write-only Go binding around an Ethereum contract.
type MEVMSwapAppTransactor struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// MEVMSwapAppFilterer is an auto generated log filtering Go binding around an Ethereum contract events.
type MEVMSwapAppFilterer struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// MEVMSwapAppSession is an auto generated Go binding around an Ethereum contract,
// with pre-set call and transact options.
type MEVMSwapAppSession struct {
	Contract     *MEVMSwapApp      // Generic contract binding to set the session for
	CallOpts     bind.CallOpts     // Call options to use throughout this session
	TransactOpts bind.TransactOpts // Transaction auth options to use throughout this session
}

// MEVMSwapAppCallerSession is an auto generated read-only Go binding around an Ethereum contract,
// with pre-set call options.
type MEVMSwapAppCallerSession struct {
	Contract *MEVMSwapAppCaller // Generic contract caller binding to set the session for
	CallOpts bind.CallOpts      // Call options to use throughout this session
}

// MEVMSwapAppTransactorSession is an auto generated write-only Go binding around an Ethereum contract,
// with pre-set transact options.
type MEVMSwapAppTransactorSession struct {
	Contract     *MEVMSwapAppTransactor // Generic contract transactor binding to set the session for
	TransactOpts bind.TransactOpts      // Transaction auth options to use throughout this session
}

// MEVMSwapAppRaw is an auto generated low-level Go binding around an Ethereum contract.
type MEVMSwapAppRaw struct {
	Contract *MEVMSwapApp // Generic contract binding to access the raw methods on
}

// MEVMSwapAppCallerRaw is an auto generated low-level read-only Go binding around an Ethereum contract.
type MEVMSwapAppCallerRaw struct {
	Contract *MEVMSwapAppCaller // Generic read-only contract binding to access the raw methods on
}

// MEVMSwapAppTransactorRaw is an auto generated low-level write-only Go binding around an Ethereum contract.
type MEVMSwapAppTransactorRaw struct {
	Contract *MEVMSwapAppTransactor // Generic write-only contract binding to access the raw methods on
}

// NewMEVMSwapApp creates a new instance of MEVMSwapApp, bound to a specific deployed contract.
func NewMEVMSwapApp(address common.Address, backend bind.ContractBackend) (*MEVMSwapApp, error) {
	contract, err := bindMEVMSwapApp(address, backend, backend, backend)
	if err != nil {
		return nil, err
	}
	return &MEVMSwapApp{MEVMSwapAppCaller: MEVMSwapAppCaller{contract: contract}, MEVMSwapAppTransactor: MEVMSwapAppTransactor{contract: contract}, MEVMSwapAppFilterer: MEVMSwapAppFilterer{contract: contract}}, nil
}

// NewMEVMSwapAppCaller creates a new read-only instance of MEVMSwapApp, bound to a specific deployed contract.
func NewMEVMSwapAppCaller(address common.Address, caller bind.ContractCaller) (*MEVMSwapAppCaller, error) {
	contract, err := bindMEVMSwapApp(address, caller, nil, nil)
	if err != nil {
		return nil, err
	}
	return &MEVMSwapAppCaller{contract: contract}, nil
}

// NewMEVMSwapAppTransactor creates a new write-only instance of MEVMSwapApp, bound to a specific deployed contract.
func NewMEVMSwapAppTransactor(address common.Address, transactor bind.ContractTransactor) (*MEVMSwapAppTransactor, error) {
	contract, err := bindMEVMSwapApp(address, nil, transactor, nil)
	if err != nil {
		return nil, err
	}
	return &MEVMSwapAppTransactor{contract: contract}, nil
}

// NewMEVMSwapAppFilterer creates a new log filterer instance of MEVMSwapApp, bound to a specific deployed contract.
func NewMEVMSwapAppFilterer(address common.Address, filterer bind.ContractFilterer) (*MEVMSwapAppFilterer, error) {
	contract, err := bindMEVMSwapApp(address, nil, nil, filterer)
	if err != nil {
		return nil, err
	}
	return &MEVMSwapAppFilterer{contract: contract}, nil
}

// bindMEVMSwapApp binds a generic wrapper to an already deployed contract.
func bindMEVMSwapApp(address common.Address, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
	parsed, err := MEVMSwapAppMetaData.GetAbi()
	if err != nil {
		return nil, err
	}
	return bind.NewBoundContract(address, *parsed, caller, transactor, filterer), nil
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_MEVMSwapApp *MEVMSwapAppRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _MEVMSwapApp.Contract.MEVMSwapAppCaller.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_MEVMSwapApp *MEVMSwapAppRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _MEVMSwapApp.Contract.MEVMSwapAppTransactor.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_MEVMSwapApp *MEVMSwapAppRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _MEVMSwapApp.Contract.MEVMSwapAppTransactor.contract.Transact(opts, method, params...)
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_MEVMSwapApp *MEVMSwapAppCallerRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _MEVMSwapApp.Contract.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_MEVMSwapApp *MEVMSwapAppTransactorRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _MEVMSwapApp.Contract.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_MEVMSwapApp *MEVMSwapAppTransactorRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _MEVMSwapApp.Contract.contract.Transact(opts, method, params...)
}

// DecodeMemo is a free data retrieval call binding the contract method 0xa06ea8bc.
//
// Solidity: function decodeMemo(bytes data) pure returns(address, bytes)
func (_MEVMSwapApp *MEVMSwapAppCaller) DecodeMemo(opts *bind.CallOpts, data []byte) (common.Address, []byte, error) {
	var out []interface{}
	err := _MEVMSwapApp.contract.Call(opts, &out, "decodeMemo", data)

	if err != nil {
		return *new(common.Address), *new([]byte), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)
	out1 := *abi.ConvertType(out[1], new([]byte)).(*[]byte)

	return out0, out1, err

}

// DecodeMemo is a free data retrieval call binding the contract method 0xa06ea8bc.
//
// Solidity: function decodeMemo(bytes data) pure returns(address, bytes)
func (_MEVMSwapApp *MEVMSwapAppSession) DecodeMemo(data []byte) (common.Address, []byte, error) {
	return _MEVMSwapApp.Contract.DecodeMemo(&_MEVMSwapApp.CallOpts, data)
}

// DecodeMemo is a free data retrieval call binding the contract method 0xa06ea8bc.
//
// Solidity: function decodeMemo(bytes data) pure returns(address, bytes)
func (_MEVMSwapApp *MEVMSwapAppCallerSession) DecodeMemo(data []byte) (common.Address, []byte, error) {
	return _MEVMSwapApp.Contract.DecodeMemo(&_MEVMSwapApp.CallOpts, data)
}

// EncodeMemo is a free data retrieval call binding the contract method 0xdf73044e.
//
// Solidity: function encodeMemo(address targetMRC20, bytes recipient) pure returns(bytes)
func (_MEVMSwapApp *MEVMSwapAppCaller) EncodeMemo(opts *bind.CallOpts, targetMRC20 common.Address, recipient []byte) ([]byte, error) {
	var out []interface{}
	err := _MEVMSwapApp.contract.Call(opts, &out, "encodeMemo", targetMRC20, recipient)

	if err != nil {
		return *new([]byte), err
	}

	out0 := *abi.ConvertType(out[0], new([]byte)).(*[]byte)

	return out0, err

}

// EncodeMemo is a free data retrieval call binding the contract method 0xdf73044e.
//
// Solidity: function encodeMemo(address targetMRC20, bytes recipient) pure returns(bytes)
func (_MEVMSwapApp *MEVMSwapAppSession) EncodeMemo(targetMRC20 common.Address, recipient []byte) ([]byte, error) {
	return _MEVMSwapApp.Contract.EncodeMemo(&_MEVMSwapApp.CallOpts, targetMRC20, recipient)
}

// EncodeMemo is a free data retrieval call binding the contract method 0xdf73044e.
//
// Solidity: function encodeMemo(address targetMRC20, bytes recipient) pure returns(bytes)
func (_MEVMSwapApp *MEVMSwapAppCallerSession) EncodeMemo(targetMRC20 common.Address, recipient []byte) ([]byte, error) {
	return _MEVMSwapApp.Contract.EncodeMemo(&_MEVMSwapApp.CallOpts, targetMRC20, recipient)
}

// Router02 is a free data retrieval call binding the contract method 0xbd00c9c4.
//
// Solidity: function router02() view returns(address)
func (_MEVMSwapApp *MEVMSwapAppCaller) Router02(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _MEVMSwapApp.contract.Call(opts, &out, "router02")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// Router02 is a free data retrieval call binding the contract method 0xbd00c9c4.
//
// Solidity: function router02() view returns(address)
func (_MEVMSwapApp *MEVMSwapAppSession) Router02() (common.Address, error) {
	return _MEVMSwapApp.Contract.Router02(&_MEVMSwapApp.CallOpts)
}

// Router02 is a free data retrieval call binding the contract method 0xbd00c9c4.
//
// Solidity: function router02() view returns(address)
func (_MEVMSwapApp *MEVMSwapAppCallerSession) Router02() (common.Address, error) {
	return _MEVMSwapApp.Contract.Router02(&_MEVMSwapApp.CallOpts)
}

// SystemContract is a free data retrieval call binding the contract method 0xbb88b769.
//
// Solidity: function systemContract() view returns(address)
func (_MEVMSwapApp *MEVMSwapAppCaller) SystemContract(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _MEVMSwapApp.contract.Call(opts, &out, "systemContract")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// SystemContract is a free data retrieval call binding the contract method 0xbb88b769.
//
// Solidity: function systemContract() view returns(address)
func (_MEVMSwapApp *MEVMSwapAppSession) SystemContract() (common.Address, error) {
	return _MEVMSwapApp.Contract.SystemContract(&_MEVMSwapApp.CallOpts)
}

// SystemContract is a free data retrieval call binding the contract method 0xbb88b769.
//
// Solidity: function systemContract() view returns(address)
func (_MEVMSwapApp *MEVMSwapAppCallerSession) SystemContract() (common.Address, error) {
	return _MEVMSwapApp.Contract.SystemContract(&_MEVMSwapApp.CallOpts)
}

// OnCall is a paid mutator transaction binding the contract method 0x5bcfd616.
//
// Solidity: function onCall((bytes,address,uint256) , address mrc20, uint256 amount, bytes message) returns()
func (_MEVMSwapApp *MEVMSwapAppTransactor) OnCall(opts *bind.TransactOpts, arg0 Context, mrc20 common.Address, amount *big.Int, message []byte) (*types.Transaction, error) {
	return _MEVMSwapApp.contract.Transact(opts, "onCall", arg0, mrc20, amount, message)
}

// OnCall is a paid mutator transaction binding the contract method 0x5bcfd616.
//
// Solidity: function onCall((bytes,address,uint256) , address mrc20, uint256 amount, bytes message) returns()
func (_MEVMSwapApp *MEVMSwapAppSession) OnCall(arg0 Context, mrc20 common.Address, amount *big.Int, message []byte) (*types.Transaction, error) {
	return _MEVMSwapApp.Contract.OnCall(&_MEVMSwapApp.TransactOpts, arg0, mrc20, amount, message)
}

// OnCall is a paid mutator transaction binding the contract method 0x5bcfd616.
//
// Solidity: function onCall((bytes,address,uint256) , address mrc20, uint256 amount, bytes message) returns()
func (_MEVMSwapApp *MEVMSwapAppTransactorSession) OnCall(arg0 Context, mrc20 common.Address, amount *big.Int, message []byte) (*types.Transaction, error) {
	return _MEVMSwapApp.Contract.OnCall(&_MEVMSwapApp.TransactOpts, arg0, mrc20, amount, message)
}

// OnCrossChainCall is a paid mutator transaction binding the contract method 0xde43156e.
//
// Solidity: function onCrossChainCall((bytes,address,uint256) , address mrc20, uint256 amount, bytes message) returns()
func (_MEVMSwapApp *MEVMSwapAppTransactor) OnCrossChainCall(opts *bind.TransactOpts, arg0 Context, mrc20 common.Address, amount *big.Int, message []byte) (*types.Transaction, error) {
	return _MEVMSwapApp.contract.Transact(opts, "onCrossChainCall", arg0, mrc20, amount, message)
}

// OnCrossChainCall is a paid mutator transaction binding the contract method 0xde43156e.
//
// Solidity: function onCrossChainCall((bytes,address,uint256) , address mrc20, uint256 amount, bytes message) returns()
func (_MEVMSwapApp *MEVMSwapAppSession) OnCrossChainCall(arg0 Context, mrc20 common.Address, amount *big.Int, message []byte) (*types.Transaction, error) {
	return _MEVMSwapApp.Contract.OnCrossChainCall(&_MEVMSwapApp.TransactOpts, arg0, mrc20, amount, message)
}

// OnCrossChainCall is a paid mutator transaction binding the contract method 0xde43156e.
//
// Solidity: function onCrossChainCall((bytes,address,uint256) , address mrc20, uint256 amount, bytes message) returns()
func (_MEVMSwapApp *MEVMSwapAppTransactorSession) OnCrossChainCall(arg0 Context, mrc20 common.Address, amount *big.Int, message []byte) (*types.Transaction, error) {
	return _MEVMSwapApp.Contract.OnCrossChainCall(&_MEVMSwapApp.TransactOpts, arg0, mrc20, amount, message)
}
