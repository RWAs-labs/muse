...
func TestPrecompilesStakingIsDisabled(r *runner.E2ERunner, args []string) {
	...
	stakingContract, err := staking.NewIStaking(staking.ContractAddress, r.MEVMClient)
	...
	sharesBeforeVal1, err := stakingContract.GetShares(&bind.CallOpts{}, r.MEVMAuth.From, validators[0].OperatorAddress)
	require.NoError(r, err)
	require.Equal(r, int64(0), sharesBeforeVal1.Int64())

	sharesBeforeVal2, err := stakingContract.GetShares(&bind.CallOpts{}, r.MEVMAuth.From, validators[1].OperatorAddress)
	require.NoError(r, err)
	require.Equal(r, int64(0), sharesBeforeVal2.Int64())

	// stake 3 to validator1
	tx, err := stakingContract.Stake(r.MEVMAuth, r.MEVMAuth.From, validators[0].OperatorAddress, big.NewInt(3))
	require.NoError(r, err)
	...
	tx, err = stakingContract.Unstake(r.MEVMAuth, r.MEVMAuth.From, validators[0].OperatorAddress, big.NewInt(1))
	require.NoError(r, err)
	receipt = utils.MustWaitForTxReceipt(r.Ctx, r.MEVMClient, tx, r.Logger, r.ReceiptTimeout)
	utils.RequiredTxFailed(r, receipt)
...
func TestPrecompilesStaking(r *runner.E2ERunner, args []string) {
	...
	sharesAfterVal1, err := stakingContract.GetShares(&bind.CallOpts{}, r.MEVMAuth.From, validators[0].OperatorAddress)
	require.NoError(r, err)
	...
	delegationAfterVal1, err := r.StakingClient.Delegation(r.Ctx, &types.QueryDelegationRequest{
		DelegatorAddr: sdk.AccAddress(r.MEVMAuth.From.Bytes()).String(),
	...
	sharesAfterVal2, err := stakingContract.GetShares(&bind.CallOpts{}, r.MEVMAuth.From, validators[1].OperatorAddress)
	require.NoError(r, err)
...
func CleanValidatorDelegations(r *runner.E2ERunner, stakingContract *staking.IStaking, validators []staking.Validator) {
	for _, validator := range validators {
		delegator := sdk.AccAddress(r.MEVMAuth.From.Bytes()).String()
