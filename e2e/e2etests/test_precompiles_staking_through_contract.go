...
import (
	...
	"github.com/RWAs-labs/muse/e2e/contracts/teststaking"
...
func TestPrecompilesStakingThroughContract(r *runner.E2ERunner, args []string) {
	...
	utils.MustWaitForTxReceipt(r.Ctx, r.MEVMClient, testStakingTx, r.Logger, r.ReceiptTimeout)
	...
	_, err = testStaking.Stake(r.MEVMAuth, r.MEVMAuth.From, validators[0].OperatorAddress, big.NewInt(3))
	require.Error(r, err)

	// stake 3 to validator1 using testStaking smart contract should fail because it doesn't have any balance yet
	_, err = testStaking.Stake(r.MEVMAuth, testStakingAddr, validators[0].OperatorAddress, big.NewInt(3))
	require.Error(r, err)

	r.MEVMAuth.Value = big.NewInt(1000000000000)
	...
	// fund testStaking contract with amuse
	tx, err := testStaking.DepositWMUSE(r.MEVMAuth)
	require.NoError(r, err)
	...
	tx, err = testStaking.WithdrawWMUSE(r.MEVMAuth, big.NewInt(int64(stakeAmount)))
	require.NoError(r, err)
	utils.MustWaitForTxReceipt(r.Ctx, r.MEVMClient, tx, r.Logger, r.ReceiptTimeout)
	...
	tx, err = testStaking.StakeAndRevert(r.MEVMAuth, testStakingAddr, validators[0].OperatorAddress, big.NewInt(3))
	require.NoError(r, err)
	utils.MustWaitForTxReceipt(r.Ctx, r.MEVMClient, tx, r.Logger, r.ReceiptTimeout)
	...
	tx, err = testStaking.Stake(r.MEVMAuth, testStakingAddr, validators[0].OperatorAddress, big.NewInt(1))
	require.NoError(r, err)
	...
	tx, err = testStaking.Unstake(r.MEVMAuth, testStakingAddr, validators[0].OperatorAddress, big.NewInt(1))
	require.NoError(r, err)
	receipt = utils.MustWaitForTxReceipt(r.Ctx, r.MEVMClient, tx, r.Logger, r.ReceiptTimeout)
