...
func TestERC20DepositAndCallRefund(r *runner.E2ERunner, _ []string) {
	...
	initialBal, err := r.ERC20MRC20.BalanceOf(&bind.CallOpts{}, r.EVMAddress())
	...
	_, err = r.MuseTxServer.BroadcastTx(utils.OperationalPolicyName, msg)
	require.NoError(r, err)

	// Check that the erc20 in the aborted cctx was refunded on MuseChain
