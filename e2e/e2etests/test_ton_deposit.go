...
func TestTONDeposit(r *runner.E2ERunner, args []string) {
	...
	// Get TON MRC20 balance before deposit
	balanceBefore, err := r.TONMRC20.BalanceOf(&bind.CallOpts{}, recipient)
	require.NoError(r, err)
	r.Logger.Info("Recipient's mEVM TON balance before deposit: %d", balanceBefore.Uint64())
	...
	balanceAfter, err := r.TONMRC20.BalanceOf(&bind.CallOpts{}, recipient)

	require.NoError(r, err)
	r.Logger.Info("Recipient's mEVM TON balance after deposit: %d", balanceAfter.Uint64())
