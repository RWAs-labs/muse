...
func UndelegateToBelowMinimumObserverDelegation(r *runner.E2ERunner, _ []string) {
	...
	// This works as expected , however the hardcoded value of 1muse (1000000000000000000amuse), for the `min_observer_delegation` is not ideal, for this check
	// The minimum accepted value for `MIN_SELF_DELEGATION` is also 1muse , therefore this `MsgUndelegate` message ends up removing the validator from the validator set as well.
	// NOTE : although the MIN_SELF_DELEGATION is set to 1, it does not mean 1amuse , when calculating the default power reduction is accounted for, therefore 1MUSE = 1 unit of voting power and not 1 amuse
	...
	// https://github.com/RWAs-labs/muse/issues/3550
