...
func (k msgServer) UpdateContractBytecode(
	...
	// check the contract is a mrc20
	_, found := k.GetForeignCoins(ctx, msg.ContractAddress)
	if !found {
		// check contract is wmuse connector contract
		systemContract, found := k.GetSystemContract(ctx)
		if !found {
			return nil, types.ErrSystemContractNotFound
		}
		if msg.ContractAddress != systemContract.ConnectorMevm {
			// not a mrc20 or wmuse connector contract, can't be updated
			return nil, cosmoserrors.Wrapf(
				types.ErrInvalidContract,
				"contract (%s) is neither a mrc20 nor wmuse connector",
				msg.ContractAddress,
			)
		}
	}
