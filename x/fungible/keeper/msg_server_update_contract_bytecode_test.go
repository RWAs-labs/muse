...
func TestKeeper_UpdateContractBytecode(t *testing.T) {
	t.Run("can update the bytecode from another contract", func(t *testing.T) {
		...
		mrc20 := setupGasCoin(t, ctx, k, sdkk.EvmKeeper, chainID1, "alpha", "alpha")

		// do some operation to populate the state
		_, err := k.DepositMRC20(ctx, mrc20, addr1, big.NewInt(100))
		require.NoError(t, err)
		_, err = k.DepositMRC20(ctx, mrc20, addr2, big.NewInt(200))
		require.NoError(t, err)

		// check the state
		checkState := func() {
			// state that should not change
			balance, err := k.BalanceOfMRC4(ctx, mrc20, addr1)
			require.NoError(t, err)
			require.Equal(t, int64(100), balance.Int64())
			balance, err = k.BalanceOfMRC4(ctx, mrc20, addr2)
			require.NoError(t, err)
			require.Equal(t, int64(200), balance.Int64())
			totalSupply, err := k.TotalSupplyMRC4(ctx, mrc20)
			require.NoError(t, err)
			require.Equal(t, int64(10000300), totalSupply.Int64()) // 10000000 minted on deploy
		}

		checkState()
		chainID, err := k.QueryChainIDFromContract(ctx, mrc20)
		require.NoError(t, err)
		require.Equal(t, chainID1, chainID.Int64())

		// deploy new mrc20
		newCodeAddress, err := k.DeployMRC20Contract(
			ctx,
			"beta",
			"BETA",
			18,
		...
		acct := sdkk.EvmKeeper.GetAccount(ctx, mrc20)
		...
		_, err = k.DepositMRC20(ctx, mrc20, addr1, big.NewInt(1000))
		require.NoError(t, err)
		balance, err := k.BalanceOfMRC4(ctx, mrc20, addr1)
		require.NoError(t, err)
	...
	t.Run("can update the bytecode of the wmuse connector contract", func(t *testing.T) {
		k, ctx, sdkk, _ := keepertest.FungibleKeeperWithMocks(t, keepertest.FungibleMockOptions{
			UseAuthorityMock: true,
		})
		k.GetAuthKeeper().GetModuleAccount(ctx, types.ModuleName)
		...
		// deploy a connector
		wmuse, _, _, oldConnector, _ := deploySystemContracts(t, ctx, k, sdkk.EvmKeeper)
		codeHash := codeHashFromAddress(t, ctx, k, oldConnector.Hex())

		// deploy a new connector that will become official connector
		newConnector, err := k.DeployConnectorMEVM(ctx, wmuse)
		require.NoError(t, err)
	...
	t.Run("should fail neither a mrc20 nor wmuse connector", func(t *testing.T) {
		k, ctx, sdkk, _ := keepertest.FungibleKeeperWithMocks(t, keepertest.FungibleMockOptions{
			UseAuthorityMock: true,
		})
		msgServer := keeper.NewMsgServerImpl(*k)
		...
		// can't update the bytecode of the wmuse contract
		msg := types.NewMsgUpdateContractBytecode(
			admin,
			wmuse.Hex(),
			sample.Hash().Hex(),
	...
	t.Run("should fail if can't set account with new bytecode", func(t *testing.T) {
		...
		k.SetSystemContract(ctx, types.SystemContract{
			ConnectorMevm: contractAddr.String(),
		})
