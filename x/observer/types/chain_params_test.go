...
func TestChainParamsEqual(t *testing.T) {
	...
	// MuseTokenContractAddress matters
	cp = copyParams(params)
	cp.MuseTokenContractAddress = "0x_something_else"
	require.False(t, types.ChainParamsEqual(*params, *cp))
...
func (s *UpdateChainParamsSuite) SetupTest() {
	s.museParams = &types.ChainParams{
		ConfirmationCount:           0,
		GasPriceTicker:              0,
		InboundTicker:               0,
		OutboundTicker:              0,
		WatchUtxoTicker:             0,
		MuseTokenContractAddress:    "0x0000000000000000000000000000000000000000",
	...
	s.evmParams = &types.ChainParams{
		...
		OutboundTicker:              1,
		WatchUtxoTicker:             0,
		MuseTokenContractAddress:    "0xA8D5060feb6B456e886F023709A2795373691E63",
...
func (s *UpdateChainParamsSuite) TestValidParams() {
	require.NoError(s.T(), s.museParams.Validate())
...
func (s *UpdateChainParamsSuite) TestCoreContractAddresses() {
	cp := *s.evmParams
	cp.MuseTokenContractAddress = "0x123"
	require.Error(s.T(), cp.Validate())

	cp = *s.evmParams
	cp.MuseTokenContractAddress = "733aB8b06DDDEf27Eaa72294B0d7c9cEF7f12db9"
	require.Error(s.T(), cp.Validate())
