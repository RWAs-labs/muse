...
func NewMsgVoteTSS(creator string, pubkey string, keygenMuseHeight int64, status chains.ReceiveStatus) *MsgVoteTSS {
	return &MsgVoteTSS{
		Creator:          creator,
		TssPubkey:        pubkey,
		KeygenMuseHeight: keygenMuseHeight,
		Status:           status,
	}
}
...
func (msg *MsgVoteTSS) Digest() string {
	...
	return fmt.Sprintf("%d-%s-%s", msg.KeygenMuseHeight, msg.TssPubkey, "tss-keygen")
}
