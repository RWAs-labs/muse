...
func SimulateMsgVoteTSS(k keeper.Keeper) simtypes.Operation {
	return func(
		...
		ballotVotesTransitionMatrix, yesVotePercentageArray, ballotVotesState := musesimulation.TSSVoteSimulationMatrix()
		...
		msg := types.MsgVoteTSS{
			...
			KeygenMuseHeight: keygen.BlockNumber,
			Status:           vote,
		}
		...
		simAccount, firstVoter, err := musesimulation.GetRandomNodeAccount(r, ctx, k, accs)
		if err != nil {
		...
		for voteCount, nodeAccount := range nodeAccounts {
			...
			observerAccount, err := musesimulation.GetSimAccount(nodeAccount.Operator, accs)
			if err != nil {
				continue
			}
