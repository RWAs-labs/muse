...
func (k Keeper) SetTSSHistory(ctx sdk.Context, tss types.TSS) {
	...
	b := k.cdc.MustMarshal(&tss)
	store.Set(types.KeyPrefix(fmt.Sprintf("%d", tss.FinalizedMuseHeight)), b)
}

// GetHistoricalTssByFinalizedHeight Returns the TSS address the specified finalized muse height
// Finalized muse height is the muse block height at which the voting for the generation of a new TSS is finalized
func (k Keeper) GetHistoricalTssByFinalizedHeight(ctx sdk.Context, finalizedMuseHeight int64) (types.TSS, bool) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.TSSHistoryKey))
	b := store.Get(types.KeyPrefix(fmt.Sprintf("%d", finalizedMuseHeight)))
	if b == nil {
...
func (k Keeper) GetPreviousTSS(ctx sdk.Context) (val types.TSS, found bool) {
	...
	// Sort tssList by FinalizedMuseHeight
