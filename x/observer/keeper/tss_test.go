...
func TestKeeper_GetHistoricalTssByFinalizedHeight(t *testing.T) {
	...
	_, found := k.GetHistoricalTssByFinalizedHeight(ctx, tssList[r].FinalizedMuseHeight)
	require.False(t, found)

	for _, tss := range tssList {
		k.SetTSSHistory(ctx, tss)
	}
	tss, found := k.GetHistoricalTssByFinalizedHeight(ctx, tssList[r].FinalizedMuseHeight)
	require.True(t, found)
...
func TestKeeper_TssHistory(t *testing.T) {
	t.Run("Get tss history paginated by limit", func(t *testing.T) {
		...
		sort.Slice(rst, func(i, j int) bool {
			return rst[i].FinalizedMuseHeight < rst[j].FinalizedMuseHeight
		})
