...
func (k Keeper) VoteOnInboundBallot(
	...
	// check if we want to send MUSE to external chain, but there is no MUSE token.
	if receiverChain.IsExternalChain() {
		...
		if coreParams.MuseTokenContractAddress == "" && coinType == coin.CoinType_Muse {
			return false, false, types.ErrInvalidMuseCoinTypes
		}
	}
