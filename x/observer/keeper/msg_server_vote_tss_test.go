...
func TestMsgServer_VoteTSS(t *testing.T) {
	...
	t.Run("can create a new ballot, vote success and finalize", func(t *testing.T) {
		...
		require.True(t, found)
		require.Equal(t, tss.KeyGenMuseHeight, int64(42))
		require.Equal(t, tss.FinalizedMuseHeight, finalizingHeight)
	})
	...
	t.Run("can create a new ballot, vote without finalizing,then finalize newer ballot", func(t *testing.T) {
		...
		require.True(t, found)
		require.Equal(t, tssQueried.KeyGenMuseHeight, int64(52))
		require.Equal(t, tssQueried.FinalizedMuseHeight, finalizingHeight)
	...
	t.Run("unable to finalize tss if pubkey is different", func(t *testing.T) {
		...
		require.Equal(t, finalizingHeight, tssQueried.FinalizedMuseHeight)
