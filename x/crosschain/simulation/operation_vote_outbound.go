...
func SimulateVoteOutbound(k keeper.Keeper) simtypes.Operation {
	...
	ballotVotesTransitionMatrix, yesVotePercentageArray, ballotVotesState := musesimulation.OutboundVoteStatusSimulationMatrix()
	return func(
		r *rand.Rand,
		app *baseapp.BaseApp,
		ctx sdk.Context,
		...
		_, creator, _, err := musesimulation.GetRandomAccountAndObserver(r, ctx, k.GetObserverKeeper(), accs)
		if err != nil {
		...
		asset, err := musesimulation.GetAsset(ctx, k.GetFungibleKeeper(), to)
		if err != nil {
