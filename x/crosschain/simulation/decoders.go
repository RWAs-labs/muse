...
func NewDecodeStore(cdc codec.Codec) func(kvA, kvB kv.Pair) string {
	return func(kvA, kvB kv.Pair) string {
		...
		case bytes.Equal(kvA.Key, types.KeyPrefix(types.MuseAccountingKey)):
			var museAccountingA, museAccountingB types.MuseAccounting
			cdc.MustUnmarshal(kvA.Value, &museAccountingA)
			cdc.MustUnmarshal(kvB.Value, &museAccountingB)
			return fmt.Sprintf(
				"key %s value A %v value B %v",
				types.MuseAccountingKey,
				museAccountingA,
				museAccountingB,
			)
