...
type crosschainKeeper interface {
	...
	SetMuseAccounting(ctx sdk.Context, accounting types.MuseAccounting)
}

// MigrateStore migrates the x/crosschain module state from the consensus version 4 to 5
// It resets the aborted muse amount to use the inbound tx amount instead in situations where the outbound cctx is never created.
func MigrateStore(ctx sdk.Context, crosschainKeeper crosschainKeeper, observerKeeper types.ObserverKeeper) error {
	err := SetMuseAccounting(ctx, crosschainKeeper, observerKeeper)
	if err != nil {
		return err
	}
...
func SetMuseAccounting(
	ctx sdk.Context,
	crosschainKeeper crosschainKeeper,
	observerKeeper types.ObserverKeeper,
) error {
	ccctxList := crosschainKeeper.GetAllCrossChainTx(ctx)
	abortedAmountMuse := sdkmath.ZeroUint()
	for _, cctx := range ccctxList {
		if cctx.CctxStatus.Status == types.CctxStatus_Aborted {
			...
			case coin.CoinType_ERC20:
				{
					...
					// Auto refunds are done for ERC20 cctx's when the receiver chain is a muse chain.
			...
			case coin.CoinType_Muse:
				{
					// add the required amount into the muse accounting.
					...
					abortedAmountMuse = abortedAmountMuse.Add(abortedValue)
					cctx.CctxStatus.IsAbortRefunded = false
				}
	...
	crosschainKeeper.SetMuseAccounting(ctx, types.MuseAccounting{AbortedMuseAmount: abortedAmountMuse})
