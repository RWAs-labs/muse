...
// https://github.com/RWAs-labs/muse/issues/2234
...
func ValidateHashForChain(hash string, chainID int64) error {
	if chains.IsEthereumChain(chainID, []chains.Chain{}) || chains.IsMuseChain(chainID, []chains.Chain{}) {
...
func ValidateAddressForChain(address string, chainID int64) error {
	// we do not validate the address for muse chain as the address field can be btc or eth address
	if chains.IsMuseChain(chainID, []chains.Chain{}) {
		return nil
	}
