...
import (
	...
	"github.com/RWAs-labs/muse/pkg/gas"
)
...
func WhitelistERC20CmdCCTX(
	...
	// we use the deployed mrc20 contract address to generate a unique index
	// since other parts of the system may use the mrc20 for the index, we add a message specific suffix
	hash := crypto.Keccak256Hash(mrc20Address.Bytes(), []byte("WhitelistERC20"))
...
func MigrateFundCmdCCTX(
	...
	case chains.IsEVMChain(chainID, additionalStaticChainInfo):
		ethAddressOld, err := musecrypto.GetTSSAddrEVM(currentTSSPubKey)
		if err != nil {
			return CrossChainTx{}, err
		}
		ethAddressNew, err := musecrypto.GetTSSAddrEVM(newTSSPubKey)
		if err != nil {
			return CrossChainTx{}, err
		}
	...
	case chains.IsBitcoinChain(chainID, additionalStaticChainInfo):
		...
		btcAddressOld, err := musecrypto.GetTSSAddrBTC(currentTSSPubKey, bitcoinNetParams)
		if err != nil {
			return CrossChainTx{}, err
		}
		btcAddressNew, err := musecrypto.GetTSSAddrBTC(newTSSPubKey, bitcoinNetParams)
		if err != nil {
			return CrossChainTx{}, err
		}
...
func newCmdCCTX(
	...
	return CrossChainTx{
		...
		InboundParams: &InboundParams{
			...
			// any inbound initiated from MuseChain is deemed safely confirmed
		...
		OutboundParams: []*OutboundParams{
			{
				...
				// museclient should ALWAYS use SAFE confirmation mode to confirm a CMD tx.
				ConfirmationMode: ConfirmationMode_SAFE,
			},
		},
	}
