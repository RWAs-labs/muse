...
func TestWhitelistERC20CmdCCTX(t *testing.T) {
	t.Run("returns a new CCTX for whitelisting ERC20 tokens", func(t *testing.T) {
		// ARRANGE
		creator := sample.AccAddress()
		mrc20Address := sample.EthAddress()
		...
		cctxDifferentMRC20Address := types.WhitelistERC20CmdCCTX(
			creator,
			sample.EthAddress(),
			erc20Address,
			custodyAddress,
		...
		// check mrc20 address produces unique index
		require.NotEqual(t, cctx.Index, cctxDifferentMRC20Address.Index)
	})
}
