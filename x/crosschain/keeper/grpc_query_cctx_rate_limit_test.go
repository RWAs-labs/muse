...
func createTestRateLimiterFlags(
	...
	return &types.RateLimiterFlags{
		Enabled: true,
		Window:  window, // for instance: 500 muse blocks, 50 minutes
		Rate:    rate,
...
func setupForeignCoins(
	...
	ctx sdk.Context,
	zk keepertest.MuseKeepers,
	mrc20ETH, mrc20BTC, mrc20USDT, assetUSDT string,
) {
	// set foreign coins
	fCoins := sample.ForeignCoinList(t, mrc20ETH, mrc20BTC, mrc20USDT, assetUSDT)
...
func TestKeeper_RateLimiterInput(t *testing.T) {
	...
	// create sample mrc20 addresses for ETH, BTC, USDT
	mrc20ETH := sample.EthAddress().Hex()
	mrc20BTC := sample.EthAddress().Hex()
	mrc20USDT := sample.EthAddress().Hex()
	...
	tests := []struct {
		...
		},
		{
			...
			expectedPastCctxsValue: sdkmath.NewInt(200).
				Mul(sdkmath.NewInt(1e18)).
				String(),
			// 400 * 0.5 MUSE, ignore Eth chain reverted cctxs
			expectedPendingCctxsValue: sdkmath.NewInt(300).
				Mul(sdkmath.NewInt(1e18)).
				String(),
			// 100 * (2.5 + 0.5) MUSE
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			setupForeignCoins(t, ctx, zk, mrc20ETH, mrc20BTC, mrc20USDT, assetUSDT)
...
func TestKeeper_ListPendingCctxWithinRateLimit(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedWithdrawRate:   sdkmath.NewInt(3e18).String(), // 3 MUSE, (2.5 + 0.5) per block
		...
		},
		{
			...
			expectedWithdrawRate: sdkmath.NewInt(5e17).
				String(),
			// 0.5 MUSE per block, only btc cctxs should be counted
			rateLimitExceeded: false,
		},
		{
		...
		{
			name: "set a low rate (rate < 2.4 MUSE) to exceed rate limit in backward loop",
			rateLimitFlags: createTestRateLimiterFlags(
				500,
				math.NewUint(2*1e18),
				mrc20ETH,
		...
		{
			name: "set a median rate (2.4 MUSE < rate < 3 MUSE) to exceed rate limit in forward loop",
			rateLimitFlags: createTestRateLimiterFlags(
				500,
		...
		},
		{
			...
			//  give a block rate of 2 MUSE/block, the max value allowed should be 100 * 2 = 200 MUSE
			rateLimitFlags: createTestRateLimiterFlags(
				50,
				math.NewUint(2*1e18),
				mrc20ETH,
