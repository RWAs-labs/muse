...
func (k Keeper) IterateAndUpdateCCTXGasPrice(
	...
	crosschainFlags, found := k.museObserverKeeper.GetCrosschainFlags(ctx)
...
func CheckAndUpdateCCTXGasPriceBTC(
	...
	// 2. museclient can't replace stuck tx individually, it only gives more funds to the last stuck tx (child tx)
	// 3. updating all pending CCTXs to the same 'mediaGasPrice' number simplyfies the calculation in museclient
	...
	// museclient can't figure out whether the gas price has been updated or not, so setting the
...
func IsCCTXGasPriceUpdateSupported(chainID int64, additionalChains []chains.Chain) bool {
	switch {
	case chains.IsMuseChain(chainID, additionalChains):
		return false
	case chains.IsEVMChain(chainID, additionalChains),
