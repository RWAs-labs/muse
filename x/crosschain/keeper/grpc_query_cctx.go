...
func (k Keeper) MuseAccounting(
	c context.Context,
	_ *types.QueryMuseAccountingRequest,
) (*types.QueryMuseAccountingResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)
	amount, found := k.GetMuseAccounting(ctx)
	if !found {
		return nil, status.Error(codes.Internal, "aborted muse amount not found")
	}
	return &types.QueryMuseAccountingResponse{
		AbortedMuseAmount: amount.AbortedMuseAmount.String(),
	}, nil
}
