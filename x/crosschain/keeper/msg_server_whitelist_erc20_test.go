...
func TestKeeper_WhitelistERC20(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mrc20 := res.Mrc20Address
			cctxIndex := res.CctxIndex

			// check mrc20 and cctx created
			assertContractDeployment(t, sdkk.EvmKeeper, ctx, ethcommon.HexToAddress(mrc20))
			fc, found := zk.FungibleKeeper.GetForeignCoins(ctx, mrc20)
			...
			gasLimit, err := zk.FungibleKeeper.QueryGasLimit(ctx, ethcommon.HexToAddress(mrc20))
			require.NoError(t, err)
