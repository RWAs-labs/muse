...
func (k msgServer) MigrateTssFunds(
	...
	if tss.FinalizedMuseHeight >= tssHistory[len(tssHistory)-1].FinalizedMuseHeight {
...
func (k Keeper) initiateMigrateTSSFundsCCTX(
	...
	existingMigrationInfo, found := k.museObserverKeeper.GetFundMigrator(ctx, chainID)
	if found {
	...
	k.museObserverKeeper.SetFundMigrator(ctx, observertypes.TssFundMigratorInfo{
		ChainId:            chainID,
