...
func setupTssMigrationParams(
	...
	newTss := sample.Tss()
	newTss.FinalizedMuseHeight = currentTss.FinalizedMuseHeight + 1
	newTss.KeyGenMuseHeight = currentTss.KeyGenMuseHeight + 1
...
func TestMsgServer_MigrateTssFunds(t *testing.T) {
	...
	t.Run("should error if current tss is the latest", func(t *testing.T) {
		...
		observerMock.On("IsInboundEnabled", mock.Anything).Return(false)
		tss1 := sample.Tss()
		tss1.FinalizedMuseHeight = 2
		tss2 := sample.Tss()
		tss2.FinalizedMuseHeight = 1
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			require.True(t, found)
			newTss := sample.Tss()
			newTss.FinalizedMuseHeight = currentTss.FinalizedMuseHeight - 10
			newTss.KeyGenMuseHeight = currentTss.KeyGenMuseHeight - 10
			k.GetObserverKeeper().SetTSSHistory(ctx, newTss)
