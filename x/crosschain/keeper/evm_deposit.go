...
func (k Keeper) HandleEVMDeposit(ctx sdk.Context, cctx *types.CrossChainTx) (bool, error) {
	...
	if inboundCoinType == coin.CoinType_Muse {
		// In case of an error
		...
		// if coin type is Muse, this is a deposit MUSE to mEVM cctx.
		evmTxResponse, err := k.fungibleKeeper.MUSEDepositAndCallContract(
			ctx,
			sender,
			to,
		...
		// note: MRC20DepositAndCallContract is solely responsible for calling the contract and depositing tokens if needed
		// and does not include any other side effects or any logic that modifies the state directly
		tmpCtx, commit := ctx.CacheContext()

		evmTxResponse, contractCall, err := k.fungibleKeeper.MRC20DepositAndCallContract(
			tmpCtx,
			from,
			to,
			inboundAmount,
...
// we revert the cctx if a non-contract is tried to be called, if the liquidity cap is reached, or if the mrc20 is paused
func errShouldRevertCctx(err error) bool {
	return errors.Is(err, fungibletypes.ErrForeignCoinCapReached) ||
		...
		errors.Is(err, fungibletypes.ErrPausedMRC20)
}
