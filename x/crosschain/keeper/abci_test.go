...
func TestKeeper_IterateAndUpdateCctxGasPrice(t *testing.T) {
	...
	supportedChains := []chains.Chain{
		...
		{ChainId: chains.MuseChainMainnet.ChainId},
	}
	...
	createCctxWithNonceRange(t, ctx, *k, 40, 45, chains.MuseChainMainnet.ChainId, tss, zk)
...
func Test_IsCCTXGasPriceUpdateSupported(t *testing.T) {
	tt := []struct {
		...
		{
			name:      "Musechain is unsupported for gas price update",
			chainID:   chains.MuseChainMainnet.ChainId,
			isSupport: false,
		},
		{
			name:      "Musechain testnet is unsupported for gas price update",
			chainID:   chains.MuseChainTestnet.ChainId,
			isSupport: false,
		},
		{
