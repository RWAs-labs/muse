...
func (k Keeper) RateLimiterInput(
	...
	// if a cctx is an outgoing cctx that orginates from MuseChain
	...
	isCCTXOutgoing := func(cctx *types.CrossChainTx) bool {
		return chains.IsMuseChain(cctx.InboundParams.SenderChainId, k.GetAuthorityKeeper().GetAdditionalChainList(ctx))
	}
		...
		for nonce := startNonce; nonce >= 0; nonce-- {
			...
			if inWindow && isOutgoing && isPast {
				pastCctxsValue = pastCctxsValue.Add(
					types.ConvertCctxValueToAmuse(chain.ChainId, cctx, gasAssetRateMap, erc20AssetRateMap),
				)
			}
			...
			if IsPending(cctx) {
				...
				if isPast {
					...
					pendingCctxsValue = pendingCctxsValue.Add(types.ConvertCctxValueToAmuse(chain.ChainId, cctx, gasAssetRateMap, erc20AssetRateMap))
				}
			}
		}
	}
...
func (k Keeper) ListPendingCctxWithinRateLimit(
	...
	limitExceeded := false
	totalPending := uint64(0)
	totalWithdrawInAmuse := sdkmath.NewInt(0)
	cctxs := make([]*types.CrossChainTx, 0)
	foreignChains := chains.FilterChains(k.museObserverKeeper.GetSupportedChains(ctx), chains.FilterExternalChains)
	...
	// initiate block limit and window limit in amuse; build asset rate maps
	blockLimitInAmuse := sdkmath.NewIntFromBigInt(rateLimitFlags.Rate.BigInt())
	windowLimitInAmuse := blockLimitInAmuse.Mul(sdkmath.NewInt(rateLimitFlags.Window))
	gasAssetRateMap, erc20AssetRateMap := types.BuildAssetRateMapFromList(assetRates)
	...
	// if a cctx is outgoing from MuseChain
	// reverted incoming cctx has an external `SenderChainId` and should not be counted
	isCCTXOutgoing := func(cctx *types.CrossChainTx) bool {
		return chains.IsMuseChain(cctx.InboundParams.SenderChainId, k.GetAuthorityKeeper().GetAdditionalChainList(ctx))
	}
	...
	// invariant: for period of time >= `rateLimitFlags.Window`, the museclient-side average withdraw rate should be <= `blockLimitInMuse`
	// otherwise, this query should return empty result and wait for the average rate to drop below `blockLimitInMuse`
	withdrawWindow := rateLimitFlags.Window
	withdrawLimitInAmuse := windowLimitInAmuse
	if lowestPendingCctxHeight != 0 {
		...
		if pendingCctxWindow > rateLimitFlags.Window {
			...
			withdrawLimitInAmuse = blockLimitInAmuse.Mul(sdkmath.NewInt(pendingCctxWindow))
		}
	}
		...
		for nonce := startNonce; nonce >= 0; nonce-- {
			...
			if inWindow && isOutgoing &&
				types.RateLimitExceeded(
					...
					gasAssetRateMap,
					erc20AssetRateMap,
					&totalWithdrawInAmuse,
					withdrawLimitInAmuse,
				) {
				limitExceeded = true
				continue
			}
	...
	return &types.QueryListPendingCctxWithinRateLimitResponse{
		...
		CurrentWithdrawRate:   totalWithdrawInAmuse.Quo(sdkmath.NewInt(withdrawWindow)).String(),
