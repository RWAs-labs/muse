...
// ValidateOutboundMEVM processes the finalization of an outbound transaction if receiver is MEVM.
...
func (k Keeper) ValidateOutboundMEVM(
	ctx sdk.Context,
...
func (k Keeper) processFailedOutboundOnExternalChain(
	...
	switch oldStatus {
	case types.CctxStatus_PendingOutbound:
		if _, found := k.museObserverKeeper.GetSupportedChainFromChainID(ctx, cctx.InboundParams.SenderChainId); !found {
		...
		if err != nil {
			return errors.Wrap(err, "failed to validate MRC20 withdrawal")
		}
...
//
// 1. For Admin Tx or a withdrawal from Muse chain, it aborts the CCTX
//
// 2. For other CCTX
...
func (k Keeper) processFailedOutboundObservers(ctx sdk.Context, cctx *types.CrossChainTx, valueReceived string) {
	...
	// For transactions which originated from MEVM, we can process the outbound in the same block as there is no TSS signing required for the revert
	...
	if cctx.InboundParams.CoinType == coin.CoinType_Cmd {
		// if the cctx is of coin type cmd or the sender chain is muse chain, then we do not revert, the cctx is aborted
		cctx.GetCurrentOutboundParam().TxFinalizationStatus = types.TxFinalizationStatus_Executed
	...
	} else if chains.IsMuseChain(cctx.InboundParams.SenderChainId, k.GetAuthorityKeeper().GetAdditionalChainList(ctx)) {
		switch cctx.InboundParams.CoinType {
		// Try revert if the coin-type is MUSE
		case coin.CoinType_Muse:
			{
				err := k.processFailedMUSEOutboundOnMEVM(ctx, cctx)
				if err != nil {
					cctx.SetAbort(types.StatusMessages{
						StatusMessage:        "muse outbound failed and revert failed on MuseChain",
		...
		default:
			{
				...
				cctx.SetAbort(types.StatusMessages{
					...
					ErrorMessageOutbound: ccctxerror.NewCCTXErrorJSONMessage("outbound failed to be executed on connected chain", nil),
					ErrorMessageRevert:   ccctxerror.NewCCTXErrorJSONMessage(fmt.Sprintf("revert on MuseChain is not supported for cctx v1 with coin type %s", cctx.InboundParams.CoinType), nil),
				})
			}
		}
	} else {
...
// processFailedMUSEOutboundOnMEVM processes a failed MUSE outbound on MEVM
func (k Keeper) processFailedMUSEOutboundOnMEVM(ctx sdk.Context, cctx *types.CrossChainTx) error {
	...
	// create new OutboundParams for the revert. We use the fixed gas limit for revert when calling mEVM

	if err := cctx.AddRevertOutbound(fungiblekeeper.MEVMGasLimitDepositAndCall.Uint64()); err != nil {
	...
	_, err = k.fungibleKeeper.MUSERevertAndCallContract(
		ctx,
		originalSender,
		originalReceiver,
		cctx.InboundParams.SenderChainId,
	...
	if err != nil {
		return fmt.Errorf("failed MUSERevertAndCallContract: %s", err.Error())
	}
...
// - sender chain is MuseChain for regular outbound (not revert outbound)
...
func (k Keeper) processFailedOutboundV2(ctx sdk.Context, cctx *types.CrossChainTx) error {
	...
	case types.CctxStatus_PendingOutbound:
		// check the sender is MuseChain
		museChain, err := chains.MuseChainFromCosmosChainID(ctx.ChainID())
		if err != nil {
			return errors.Wrap(err, "failed to get MuseChain chainID")
		}
		if cctx.InboundParams.SenderChainId != museChain.ChainId {
			return fmt.Errorf(
				"sender chain for withdraw cctx is not MuseChain expected %d got %d",
				museChain.ChainId,
				cctx.InboundParams.SenderChainId,
			)
		}
