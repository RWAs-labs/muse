...
func TestKeeper_ValidateFailedOutboundV2(t *testing.T) {
	t.Run("cctx aborted if can't fetch MuseChain chain ID", func(t *testing.T) {
		// arrange
		k, ctx, _, _ := keepertest.CrosschainKeeper(t)
		cctx := sample.CrossChainTx(t, "test")
	...
	t.Run("cctx aborted if CCTX source chain is not MuseChain", func(t *testing.T) {
		// arrange
		k, ctx, _, _ := keepertest.CrosschainKeeper(t)
		cctx := sample.CrossChainTx(t, "test")
		cctx.ProtocolContractVersion = types.ProtocolContractVersion_V2
	...
	t.Run("cctx aborted if fail to add revert outbound", func(t *testing.T) {
		...
		cctx.ProtocolContractVersion = types.ProtocolContractVersion_V2
		cctx.CctxStatus.Status = types.CctxStatus_PendingOutbound
		cctx.InboundParams.SenderChainId = chains.MuseChainMainnet.ChainId
...
func TestKeeper_ValidateFailedOutbound(t *testing.T) {
	...
	t.Run("set failed mevm outbound of cointype ERC20 to aborted", func(t *testing.T) {
		k, ctx, _, _ := keepertest.CrosschainKeeper(t)
		cctx := sample.CrossChainTx(t, "test")
	...
	t.Run("set failed mevm outbound of cointype Gas to aborted", func(t *testing.T) {
		k, ctx, _, _ := keepertest.CrosschainKeeper(t)
		cctx := sample.CrossChainTx(t, "test")
	...
	t.Run("cctx aborted if MUSERevertAndCallContract fails", func(t *testing.T) {
		k, ctx, _, _ := keepertest.CrosschainKeeperWithMocks(t, keepertest.CrosschainMockOptions{
			UseFungibleMock: true,
		})
		fungibleMock := keepertest.GetCrosschainFungibleMock(t, k)
		receiver := sample.EthAddress()
		errorFailedMUSERevertAndCallContract := cosmoserror.New("test", 999, "failed MUSERevertAndCallContract")
		cctx := GetERC20Cctx(t, receiver, chains.Goerli, "", big.NewInt(42))
		cctx.InboundParams.CoinType = coin.CoinType_Muse
		cctx.InboundParams.SenderChainId = chains.MuseChainMainnet.ChainId
		fungibleMock.On("MUSERevertAndCallContract", mock.Anything,
			ethcommon.HexToAddress(cctx.InboundParams.Sender),
			...
			mock.Anything,
			mock.Anything).Return(nil, errorFailedMUSERevertAndCallContract).Once()
		err := k.ValidateOutboundObservers(
			ctx,
			cctx,
			observertypes.BallotStatus_BallotFinalized_FailureObservation,
