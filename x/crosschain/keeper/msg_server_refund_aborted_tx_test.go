...
func TestMsgServer_RefundAbortedCCTX(t *testing.T) {
	t.Run("successfully refund tx for coin-type gas", func(t *testing.T) {
		...
		balance, err := zk.FungibleKeeper.BalanceOfMRC4(ctx, mrc20, refundAddress)
		require.NoError(t, err)
	...
	t.Run("successfully refund tx for coin-type muse", func(t *testing.T) {
		k, ctx, sdkk, zk := keepertest.CrosschainKeeperWithMocks(t, keepertest.CrosschainMockOptions{
			UseAuthorityMock: true,
		})
		...
		k.SetMuseAccounting(
			ctx,
			crosschaintypes.MuseAccounting{AbortedMuseAmount: cctx.GetCurrentOutboundParam().Amount},
		)
		deploySystemContracts(t, ctx, zk.FungibleKeeper, sdkk.EvmKeeper)
	...
	t.Run("successfully refund tx to inbound amount if outbound is not found for coin-type muse", func(t *testing.T) {
		k, ctx, sdkk, zk := keepertest.CrosschainKeeperWithMocks(t, keepertest.CrosschainMockOptions{
			UseAuthorityMock: true,
		})
	...
	t.Run("successfully refund to optional refund address if provided", func(t *testing.T) {
		...
		cctx.InboundParams.CoinType = coin.CoinType_Muse
		k.SetCrossChainTx(ctx, *cctx)
		k.SetMuseAccounting(ctx, crosschaintypes.MuseAccounting{AbortedMuseAmount: cctx.InboundParams.Amount})
		deploySystemContracts(t, ctx, zk.FungibleKeeper, sdkk.EvmKeeper)
	...
	t.Run("successfully refund tx for coin-type ERC20", func(t *testing.T) {
		...
		balance, err := zk.FungibleKeeper.BalanceOfMRC4(ctx, mrc20Addr, refundAddress)
		require.NoError(t, err)
		require.Equal(t, cctx.GetCurrentOutboundParam().Amount.Uint64(), balance.Uint64())
		c, found := k.GetCrossChainTx(ctx, cctx.Index)
		require.True(t, found)
	...
	t.Run("fail refund tx for coin-type Muse if muse accounting object is not present", func(t *testing.T) {
		k, ctx, sdkk, zk := keepertest.CrosschainKeeperWithMocks(t, keepertest.CrosschainMockOptions{
			UseAuthorityMock: true,
		})
		...
		keepertest.MockCheckAuthorization(&authorityMock.Mock, &msg, nil)
		_, err := msgServer.RefundAbortedCCTX(ctx, &msg)
		require.ErrorContains(t, err, "unable to find muse accounting")
	})
