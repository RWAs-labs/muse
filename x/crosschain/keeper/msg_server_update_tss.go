...
func (k msgServer) UpdateTssAddress(
	...
	currentTss, found := k.museObserverKeeper.GetTSS(ctx)
	if !found {
	...
	tss, ok := k.museObserverKeeper.CheckIfTssPubkeyHasBeenGenerated(ctx, msg.TssPubkey)
	if !ok {
		return nil, errorsmod.Wrap(types.ErrUnableToUpdateTss, "tss pubkey has not been generated")
	}

	tssMigrators := k.museObserverKeeper.GetAllTssFundMigrators(ctx)
	...
	k.museObserverKeeper.RemoveAllExistingMigrators(ctx)
...
func (k *Keeper) GetChainsSupportingTSSMigration(ctx sdk.Context) []chains.Chain {
	supportedChains := k.museObserverKeeper.GetSupportedChains(ctx)
