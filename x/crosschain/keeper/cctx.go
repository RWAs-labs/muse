...
// 4. update the muse accounting
// Muse-accounting is updated aborted cctxs of cointtype muse.When a cctx is aborted it means that `GetAbortedAmount`
//of muse is locked and cannot be used.
...
// updateMuseAccounting updates the muse accounting with the amount of muse that was locked in an aborted cctx
func (k Keeper) updateMuseAccounting(
	ctx sdk.Context,
	cctx types.CrossChainTx,
) {
	if cctx.CctxStatus.Status == types.CctxStatus_Aborted &&
		...
		k.AddMuseAbortedAmount(ctx, GetAbortedAmount(cctx))
	}
}
