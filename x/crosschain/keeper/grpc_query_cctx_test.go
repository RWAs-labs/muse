...
func TestKeeper_MuseAccounting(t *testing.T) {
	t.Run("should error if muse accounting not found", func(t *testing.T) {
		k, ctx, _, _ := keepertest.CrosschainKeeper(t)
		res, err := k.MuseAccounting(ctx, nil)
		require.Error(t, err)
		require.Nil(t, res)
	})

	t.Run("should return muse accounting if found", func(t *testing.T) {
		k, ctx, _, _ := keepertest.CrosschainKeeper(t)
		k.SetMuseAccounting(ctx, types.MuseAccounting{
			AbortedMuseAmount: sdkmath.NewUint(100),
		})

		res, err := k.MuseAccounting(ctx, nil)
		require.NoError(t, err)
		require.Equal(t, &types.QueryMuseAccountingResponse{
			AbortedMuseAmount: sdkmath.NewUint(100).String(),
		}, res)
	})
}
