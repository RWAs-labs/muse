...
func (tss *TSS) PubKey() musetss.PubKey {
	if tss.fakePubKey != nil {
		return *tss.fakePubKey
	}

	pubKey, err := musetss.NewPubKeyFromECDSA(tss.privateKey.PublicKey)
...
func (tss *TSS) FakePubKey(pk any) *TSS {
	...
	if zpk, ok := pk.(musetss.PubKey); ok {
		tss.fakePubKey = &zpk
		return tss
	}
	...
	if strings.HasPrefix(raw, "musepub") {
		zpk, err := musetss.NewPubKeyFromBech32(raw)
		require.NoError(tss.t, err)
		tss.fakePubKey = &zpk
		return tss
	}
	...
	if strings.HasPrefix(raw, "0x") {
		zpk, err := musetss.NewPubKeyFromECDSAHexString(raw)
		require.NoError(tss.t, err)
		tss.fakePubKey = &zpk
		return tss
	}
