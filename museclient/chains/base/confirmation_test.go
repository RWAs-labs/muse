	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.failForeignCoinsRPC {
				ob.musecore.On("GetForeignCoinsFromAsset", mock.Anything, chain.ChainId, assetAddress).
					Maybe().
					Return(fungibletypes.ForeignCoins{}, errors.New("rpc failed"))
			} else {
				ob.musecore.On("GetForeignCoinsFromAsset", mock.Anything, chain.ChainId, assetAddress).Maybe().Return(fungibletypes.ForeignCoins{LiquidityCap: liquidityCap}, nil)
			}
