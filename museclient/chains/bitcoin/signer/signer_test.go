...
type testSuite struct {
	*Signer
	...
	musecoreClient *mocks.MusecoreClient
}
...
func Test_BroadcastOutbound(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.failTracker {
				s.musecoreClient.WithPostOutboundTracker("")
			} else {
				s.musecoreClient.WithPostOutboundTracker("ABC")
			}
...
func makeCtx(t *testing.T) context.Context {
	...
	err := app.Update(
		[]chains.Chain{chain, chains.MuseChainMainnet},
		nil,
...
func (s *testSuite) createObserver(t *testing.T) {
	...
	// create observer
	baseObserver, err := base.NewObserver(s.Chain(), params, s.musecoreClient, s.tss, 100, ts, database, baseLogger)
	require.NoError(t, err)
