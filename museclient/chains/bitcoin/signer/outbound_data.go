...
type OutboundData struct {
	...
	// this value is fed by the musecore when it bumps the gas price with gas stability pool
	feeRateLatest uint64

	// feeRateBumpped is a flag to indicate if the fee rate in CCTX is bumped by musecore
	feeRateBumped bool

	// minRelayFee is the minimum relay fee in unit of BTC
	minRelayFee float64

	// height is the MuseChain block height
	height uint64
...
func NewOutboundData(
	...
	// check if musecore has bumped the fee rate
	// 'GasPriorityFee' is always "0" for Bitcoin unless musecore bumps the fee rate
	var (
		feeRateBumped bool
		feeRateLatest uint64
	)
	if params.GasPriorityFee != "" {
		...
		if gasPriorityFee > 0 {
			...
			logger.Info().Str("latest_fee_rate", params.GasPriorityFee).Msg("fee rate is bumped by musecore")
		}
	}
