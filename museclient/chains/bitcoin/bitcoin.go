...
func (b *Bitcoin) Start(ctx context.Context) error {
	...
	newBlockChan, err := b.observer.MusecoreClient().NewBlockSubscriber(ctx)
	if err != nil {
...
// scheduleCCTX schedules pending cross-chain transactions on NEW muse blocks
...
func (b *Bitcoin) scheduleCCTX(ctx context.Context) error {
	...
	cctxList, _, err := b.observer.MusecoreClient().ListPendingCCTX(ctx, chain)
	if err != nil {
