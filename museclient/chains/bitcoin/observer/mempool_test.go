...
func Test_GetLastPendingOutbound(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			txNonce:      9,
			pendingNonce: 9, // this museclient completely lost track of the tx, pending nonce hasn't increased to 10 yet
			pendingNonces: &observertypes.PendingNonces{
				NonceLow:  9,
				NonceHigh: 10,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// mock musecore client response
			if tt.pendingNonces != nil {
				ob.musecore.On("GetPendingNoncesByChain", mock.Anything, mock.Anything).
					Maybe().
					Return(*tt.pendingNonces, nil)
			} else {
				empty := observertypes.PendingNonces{}
				ob.musecore.On("GetPendingNoncesByChain", mock.Anything, mock.Anything).Maybe().Return(empty, errors.New("failed"))
			}
...
func mockAndRefreshPendingNonce(t *testing.T, s *testSuite, txid string, nonce uint64) {
	...
	s.musecore.On("GetPendingNoncesByChain", mock.Anything, mock.Anything).Return(*pendingNonces, nil).Once()
