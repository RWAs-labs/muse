...
import (
	...
	connectorevm "github.com/RWAs-labs/protocol-contracts/pkg/museconnector.base.sol"
...
func (signer *Signer) SignConnectorOnReceive(ctx context.Context, txData *OutboundData) (*ethtypes.Transaction, error) {
	...
	museConnectorABI, err := connectorevm.MuseConnectorBaseMetaData.GetAbi()
	if err != nil {
		return nil, errors.Wrap(err, "unable to get MuseConnectorMEVMMetaData ABI")
	}

	data, err = museConnectorABI.Pack("onReceive",
		txData.sender.Bytes(),
		txData.srcChainID,
		txData.to,
		txData.amount,
...
func (signer *Signer) SignConnectorOnRevert(ctx context.Context, txData *OutboundData) (*ethtypes.Transaction, error) {
	...
	data, err = museConnectorABI.Pack("onRevert",
		txData.sender,
		txData.srcChainID,
		txData.to.Bytes(),
		txData.toChainID,
