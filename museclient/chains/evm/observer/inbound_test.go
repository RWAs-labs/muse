...
func Test_CheckAndVoteInboundTokenMuse(t *testing.T) {
	// load archived MuseSent inbound, receipt and cctx
	...
	t.Run("should pass for archived inbound, receipt and cctx", func(t *testing.T) {
		...
		ballot, err := ob.checkAndVoteInboundTokenMuse(ob.ctx, tx, receipt, false)
	...
	t.Run("should fail on unconfirmed inbound", func(t *testing.T) {
		...
		_, err := ob.checkAndVoteInboundTokenMuse(ob.ctx, tx, receipt, false)
		require.ErrorContains(t, err, "not been confirmed")
	})
	t.Run("should not act if no MuseSent event", func(t *testing.T) {
		ob := newTestSuite(t)
		...
		receipt.Logs = receipt.Logs[:2] // remove MuseSent event
		require.NoError(t, common.ValidateEvmTransaction(tx))

		ob.WithLastBlock(receipt.BlockNumber.Uint64() + ob.chainParams.InboundConfirmationSafe())

		ballot, err := ob.checkAndVoteInboundTokenMuse(ob.ctx, tx, receipt, true)
		require.NoError(t, err)
		require.Equal(t, "", ballot)
	})
	t.Run("should not act if emitter is not MuseConnector", func(t *testing.T) {
		...
		// ACT
		_, err := ob.checkAndVoteInboundTokenMuse(ob.ctx, tx, receipt, true)
...
func Test_BuildInboundVoteMsgForMuseSentEvent(t *testing.T) {
	// load archived MuseSent receipt
	// https://etherscan.io/tx/0xf3935200c80f98502d5edc7e871ffc40ca898e134525c42c2ae3cbc5725f9d76
	chainID := int64(1)
	inboundHash := "0xf3935200c80f98502d5edc7e871ffc40ca898e134525c42c2ae3cbc5725f9d76"
	receipt := testutils.LoadEVMInboundReceipt(t, TestDataDir, chainID, inboundHash, coin.CoinType_Muse)
	cctx := testutils.LoadCctxByInbound(t, chainID, coin.CoinType_Muse, inboundHash)

	// parse MuseSent event
	ob := newTestSuite(t)

	connector := mocks.MockConnectorNonEth(t, chainID)
	event := testutils.ParseReceiptMuseSent(receipt, connector)
	...
	t.Run("should return vote msg for archived MuseSent event", func(t *testing.T) {
		msg := ob.buildInboundVoteMsgForMuseSentEvent(ob.appContext, event)
...
func Test_ObserveTSSReceiveInBlock(t *testing.T) {
	...
	tests := []struct {
		...
		mockMusecoreClient func(m *mocks.MusecoreClient)
		errMsg             string
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.mockMusecoreClient != nil {
				tt.mockMusecoreClient(ob.musecore)
			}
