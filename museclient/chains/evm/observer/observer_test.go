...
func getAppContext(
	...
	chainParams := map[int64]*observertypes.ChainParams{
		...
		chains.MuseChainMainnet.ChainId: ptr.Ptr(
			mocks.MockChainParams(chains.MuseChainMainnet.ChainId, 10),
		),
	}

	// feed chain params
	err := appContext.Update(
		[]chains.Chain{evmChain, chains.MuseChainMainnet},
		nil,
		chainParams,
...
func newTestSuite(t *testing.T, opts ...func(*testSuiteConfig)) *testSuite {
	...
	baseObserver, err := base.NewObserver(chain, chainParams, musecore, tss, 1000, nil, database, logger)
	require.NoError(t, err)
