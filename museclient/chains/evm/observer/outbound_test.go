...
func Test_IsOutboundProcessed(t *testing.T) {
	// load archived outbound receipt that contains MuseReceived event
	...
	t.Run("should fail if unable to parse MuseReceived event", func(t *testing.T) {
		// create evm observer and set outbound and receipt
		ob := newTestSuite(t)
		ob.setTxNReceipt(nonce, receipt, outbound)
...
func Test_IsOutboundProcessed_ContractError(t *testing.T) {
	...
	t.Run("should fail if unable to get connector/custody contract", func(t *testing.T) {
		...
		ob := newTestSuite(t)
		ob.setTxNReceipt(nonce, receipt, outbound)
		abiConnector := museconnector.MuseConnectorNonEthMetaData.ABI
		abiCustody := erc20custody.ERC20CustodyMetaData.ABI

		// set invalid connector ABI
		museconnector.MuseConnectorNonEthMetaData.ABI = "invalid abi"
		continueKeysign, err := ob.VoteOutboundIfConfirmed(ctx, cctx)
		museconnector.MuseConnectorNonEthMetaData.ABI = abiConnector // reset connector ABI
		require.ErrorContains(t, err, "error getting muse connector")
		require.True(t, continueKeysign)
...
func Test_PostVoteOutbound(t *testing.T) {
	...
	// load archived cctx, outbound and receipt for a MuseReceived event
	// https://etherscan.io/tx/0x81342051b8a85072d3e3771c1a57c7bdb5318e8caf37f5a687b7a91e50a7257f
	...
	t.Run("post vote outbound successfully", func(t *testing.T) {
		...
		// create evm client using mock musecore client and post outbound vote
		ob := newTestSuite(t)
		ob.postVoteOutbound(
			ctx,
			cctx.Index,
...
func Test_ParseMuseReceived(t *testing.T) {
	// load archived outbound receipt that contains MuseReceived event
	// https://etherscan.io/tx/0x81342051b8a85072d3e3771c1a57c7bdb5318e8caf37f5a687b7a91e50a7257f
	chainID := chains.Ethereum.ChainId
	nonce := uint64(9718)
	...
	t.Run("should parse MuseReceived event from archived outbound receipt", func(t *testing.T) {
	...
	t.Run("should fail on connector address mismatch", func(t *testing.T) {
		...
		receivedLog, revertedLog, err := parseAndCheckMuseEvent(cctx, receipt, fakeConnectorAddress, connector)
		require.ErrorContains(t, err, "error validating MuseReceived event")
		require.Nil(t, revertedLog)
	...
	t.Run("should fail on receiver address mismatch", func(t *testing.T) {
		...
		receivedLog, revertedLog, err := parseAndCheckMuseEvent(fakeCctx, receipt, connectorAddress, connector)
	...
	t.Run("should fail if no event found in receipt", func(t *testing.T) {
		// load receipt and remove MuseReceived event from logs
		receipt := testutils.LoadEVMOutboundReceipt(
			t,
			TestDataDir,
			chainID,
		...
		receipt.Logs = receipt.Logs[:1] // the 2nd log is MuseReceived event
		receivedLog, revertedLog, err := parseAndCheckMuseEvent(cctx, receipt, connectorAddress, connector)
		require.ErrorContains(t, err, "no MuseReceived/MuseReverted event")
		require.Nil(t, revertedLog)
		require.Nil(t, receivedLog)
	})
}

func Test_ParseMuseReverted(t *testing.T) {
	// load archived outbound receipt that contains MuseReverted event
	...
	receipt := testutils.LoadEVMOutboundReceipt(
		...
		outboundHash,
		coin.CoinType_Muse,
		testutils.EventMuseReverted,
	)

	t.Run("should parse MuseReverted event from archived outbound receipt", func(t *testing.T) {
		receivedLog, revertedLog, err := parseAndCheckMuseEvent(cctx, receipt, connectorAddress, connector)
		require.NoError(t, err)
		require.Nil(t, receivedLog)
	...
	t.Run("should fail on connector address mismatch", func(t *testing.T) {
		...
		fakeConnectorAddress := sample.EthAddress()
		receivedLog, revertedLog, err := parseAndCheckMuseEvent(cctx, receipt, fakeConnectorAddress, connector)
		require.ErrorContains(t, err, "error validating MuseReverted event")
		require.Nil(t, receivedLog)
		require.Nil(t, revertedLog)
	})
	t.Run("should fail on receiver address mismatch", func(t *testing.T) {
...
//
//	t.Run("should parse and check MuseReceived event from archived outbound receipt", func(t *testing.T) {
//		// load archived outbound receipt that contains MuseReceived event
//		// https://etherscan.io/tx/0x81342051b8a85072d3e3771c1a57c7bdb5318e8caf37f5a687b7a91e50a7257f
//		nonce := uint64(9718)
//		coinType := coin.CoinType_Muse
...
//	})
//	t.Run("should parse and check MuseReverted event from archived outbound receipt", func(t *testing.T) {
//		// load archived outbound receipt that contains MuseReverted event
