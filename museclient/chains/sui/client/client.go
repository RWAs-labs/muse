...
// TODO: https://github.com/RWAs-labs/muse/issues/3775
//
...
func CheckContainOwnedObject(res []*models.SuiObjectResponse) error {
	for i, obj := range res {
		if obj.Data == nil {
			return errors.Wrapf(musesui.ErrObjectOwnership, "object %d is missing data", i)
		}

		switch owner := obj.Data.Owner.(type) {
		case string:
			if owner != immutableOwner {
				return errors.Wrapf(musesui.ErrObjectOwnership, "object %d has unexpected string owner: %s", i, owner)
			}
			// Immutable is valid, continue
		case map[string]any:
			if _, isShared := owner[sharedOwner]; !isShared {
				return errors.Wrapf(musesui.ErrObjectOwnership, "object %d is not shared or immutable: owner = %+v", i, owner)
			}
			// Shared is valid, continue
		default:
			return errors.Wrapf(musesui.ErrObjectOwnership, "object %d has unknown owner type: %+v", i, obj.Data.Owner)
		}
	}
