...
const (
	...
	// TODO: move to config
	// https://github.com/RWAs-labs/muse/issues/3086
...
// Sends PostVoteInbound to musecore
...
func (ob *Observer) voteDeposit(ctx context.Context, inbound inboundData, seqno uint32) (string, error) {
	const (
		...
		retryGasLimit = musecore.PostVoteInboundExecutionGasLimit
	)

	var (
		operatorAddress = ob.MusecoreClient().GetKeys().GetOperatorAddress()
...
func (ob *Observer) inboundComplianceCheck(inbound inboundData) (restricted bool) {
	...
	if !museclientconfig.ContainRestrictedAddress(addresses...) {
		return false
	}
