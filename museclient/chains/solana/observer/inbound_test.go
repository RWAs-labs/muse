...
func Test_FilterInboundEventAndVote(t *testing.T) {
	...
	musecoreClient.WithKeys(&keys.Keys{}).WithMuseChain().WithPostVoteInbound("", "")

	musecoreClient.MockGetCctxByHash(nil)
	musecoreClient.WithPostVoteInbound(sample.MuseIndex(t), mock.Anything)
	musecoreClient.MockGetCctxByHash(nil)
	musecoreClient.MockGetBallotByID(mock.Anything, nil)
