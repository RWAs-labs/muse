...
import (
	...
	musemath "github.com/RWAs-labs/muse/pkg/math"
)
...
const (
	...
	// The gas fee formula 'gasFee = gasPrice * gasLimit' won't fit Solana in the MRC20 SOL contract.
...
func (ob *Observer) PostGasPrice(ctx context.Context) error {
	...
	medianFee := musemath.SliceMedianValue(priorityFees, true)

	// there is no Ethereum-like gas price in Solana, we only post priority fee for now
	_, err = ob.MusecoreClient().PostVoteGasPrice(ctx, ob.Chain(), 1, medianFee, slot)
	if err != nil {
