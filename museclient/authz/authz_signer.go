...
// init initializes the signers map with all the crosschain tx types using the MuseClientGranteeKey
func init() {
	signersList := make(map[string]Signer)
	for _, tx := range crosschaintypes.GetAllAuthzMuseclientTxTypes() {
		signersList[tx] = Signer{KeyType: authz.MuseClientGranteeKey}
	}
	signers = signersList
}
