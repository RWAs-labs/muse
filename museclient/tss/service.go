...
import (
	...
	thorcommon "github.com/RWAs-labs/go-tss/common"
	...
	"github.com/RWAs-labs/muse/museclient/tss/ratelimit"
...
func WithMetrics(ctx context.Context, musecore Musecore, m *Metrics) Opt {
	return func(cfg *serviceConfig, _ zerolog.Logger) error {
		keygen, err := musecore.GetKeyGen(ctx)
		if err != nil {
...
func (s *Service) blameFailure(
	...
	museHash, err := s.musecore.PostVoteBlameData(ctx, &res.Blame, chainID, index)
	if err != nil {
	...
	s.logger.Info().
		Fields(lf).
		Str("tss.blame_tx_hash", museHash).
		Msg("Posted blame data to musecore")
