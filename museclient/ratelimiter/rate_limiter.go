...
type Output struct {
	...
	// the current withdraw rate (amuse/block) within the current sliding window
...
func ApplyRateLimiter(input *Input, window int64, rate sdkmath.Uint) *Output {
	// block limit and the window limit in amuse
	blockLimitInAmuse := sdkmath.NewIntFromBigInt(rate.BigInt())
	windowLimitInAmuse := blockLimitInAmuse.Mul(sdkmath.NewInt(window))

	// invariant: for period of time >= `window`, the museclient-side average withdraw rate should be <= `blockLimitInMuse`
	// otherwise, museclient should wait for the average rate to drop below `blockLimitInMuse`
	...
	totalWithdrawInAmuse := input.PastCctxsValue.Add(input.PendingCctxsValue)
	limitExceeded := totalWithdrawInAmuse.GT(withdrawLimitInAmuse)
	...
	return &Output{
		...
		CurrentWithdrawRate:   totalWithdrawInAmuse.Quo(sdkmath.NewInt(withdrawWindow)),
