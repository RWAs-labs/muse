...
If the observation is successful, the difference between muse burned
...
MuseChain for the MRC20 of the gas token of the receiver chain. The MRC20
tokens are then
...
If the receiver chain is MuseChain, `HandleEVMDeposit` is called. If the
tokens being deposited are MUSE, `MintMuseToEVMAccount` is called and the
tokens are minted to the receiver account on MuseChain. If the tokens being
deposited are gas tokens or ERC20 of a connected chain, MRC20's `deposit`
method is called and the tokens are deposited to the receiver account on
MuseChain. If the message is not empty, system contract's `depositAndCall`
method is also called and an omnichain contract on MuseChain is executed.
...
with the CCTX are swapped using a Uniswap V2 contract instance on MuseChain
for the MRC20 of the gas token of the receiver chain. The MRC20 tokens are
...
stateDiagram-v2
	...
	PendingInbound --> evm_deposit_success: Receiver is MuseChain
