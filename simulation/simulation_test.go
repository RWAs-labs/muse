...
import (
	...
	musesimulation "github.com/RWAs-labs/muse/simulation"
...
func init() {
	musesimulation.GetSimulatorFlags()
}
...
const (
	...
	//github.com/RWAs-labs/muse/issues/3004
...
func TestAppStateDeterminism(t *testing.T) {
	if !musesimulation.FlagEnabledValue {
		t.Skip("skipping application simulation")
	}

	config := musesimulation.NewConfigFromFlags()
	...
	appOptions[server.FlagInvCheckPeriod] = musesimulation.FlagPeriodValue
	...
	for i := 0; i < numSeeds; i++ {
		...
		for j := 0; j < numTimesToRunPerSeed; j++ {
			db, dir, logger, _, err := cosmossimutils.SetupSimulation(
				...
				SimDBBackend,
				SimDBName,
				musesimulation.FlagVerboseValue,
				musesimulation.FlagEnabledValue,
			)
			require.NoError(t, err)
			appOptions[flags.FlagHome] = dir

			simApp, err := musesimulation.NewSimApp(
				logger,
				db,
				appOptions,
				interBlockCacheOpt(),
...
func TestAppImportExport(t *testing.T) {
	...
	err = musesimulation.CheckExportSimulation(simApp, config, simParams)
	require.NoError(t, err)
	...
	newSimApp, err := musesimulation.NewSimApp(
		logger,
		newDB,
		appOptions,
		interBlockCacheOpt(),
	...
	storeKeysPrefixes := []StoreKeysPrefixes{
		...
		// TODO:https://github.com/RWAs-labs/muse/issues/3263
