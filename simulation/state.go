...
import (
	...
	musechains "github.com/RWAs-labs/muse/pkg/chains"
...
func extractObserverGenesisState(
	...
	chains := musechains.DefaultChainsList()
...
// - A random connector mevm address
...
func extractFungibleGenesisState(
	...
	fungibleState.SystemContract = &fungibletypes.SystemContract{
		...
		ConnectorMevm:  sample.EthAddressFromRand(r).String(),
	...
	for _, chain := range chains {
		foreignCoin := fungibletypes.ForeignCoins{
			...
			Mrc20ContractAddress: sample.EthAddressFromRand(r).String(),
			Decimals:             18,
			Paused:               false,
