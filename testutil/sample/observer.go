...
func ChainParams(chainID int64) *types.ChainParams {
	...
	return &types.ChainParams{
		...
		MuseTokenContractAddress:    EthAddress().String(),
...
func ChainParamsFromRand(r *rand.Rand, chainID int64) *types.ChainParams {
	...
	return &types.ChainParams{
		...
		MuseTokenContractAddress:    EthAddressFromRand(r).String(),
...
func TSSFromRand(r *rand.Rand) (types.TSS, error) {
	...
	pk, err := musecrypto.NewPubKey(spk)
	if err != nil {
		return types.TSS{}, err
	}
	...
	return types.TSS{
		...
		FinalizedMuseHeight: r.Int63(),
		KeyGenMuseHeight:    r.Int63(),
	}, nil
}

// TODO: rename to TSS
// https://github.com/RWAs-labs/muse/issues/3098
func Tss() types.TSS {
...
func TssList(n int) (tssList []types.TSS) {
	for i := 0; i < n; i++ {
		tss := Tss()
		tss.FinalizedMuseHeight = tss.FinalizedMuseHeight + int64(i)
		tss.KeyGenMuseHeight = tss.KeyGenMuseHeight + int64(i)
