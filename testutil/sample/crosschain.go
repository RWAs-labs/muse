...
func RateLimiterFlagsFromRand(r *rand.Rand) types.RateLimiterFlags {
	return types.RateLimiterFlags{
		...
		Conversions: []types.Conversion{
			{
				Mrc20: EthAddressFromRand(r).Hex(),
				Rate:  sdkmath.LegacyNewDec(r.Int63()),
			},
			{
				Mrc20: EthAddressFromRand(r).Hex(),
...
func CrossChainTx(t *testing.T, index string) *types.CrossChainTx {
	...
	return &types.CrossChainTx{
		...
		MuseFees:                sdkmath.NewUint(uint64(r.Int63())),
...
func MuseAccounting(t *testing.T, index string) types.MuseAccounting {
	r := newRandFromStringSeed(t, index)
	return types.MuseAccounting{
		AbortedMuseAmount: sdkmath.NewUint(uint64(r.Int63())),
	}
}
...
func CoinTypeFromRand(r *rand.Rand) coin.CoinType {
	coinTypes := []coin.CoinType{coin.CoinType_Gas, coin.CoinType_ERC20, coin.CoinType_Muse}
...
func MRC20Withdrawal(to []byte, value *big.Int) *mrc20.MRC20Withdrawal {
	return &mrc20.MRC20Withdrawal{
		From:            EthAddress(),
		To:              to,
		Value:           value,
...
func readMuseReceipt(t *testing.T, name string) ethtypes.Receipt {
	receipt, err := testdata.ReadMuseReceipt(name)
	require.NoError(t, err)
	return receipt
}

// InvalidMRC20WithdrawToExternalReceipt is a receipt for a invalid MRC20 withdrawal to an external address
// receiver is 1EYVvXLusCxtVuEwoYvWRyN5EZTXwPVvo3
func InvalidMRC20WithdrawToExternalReceipt(t *testing.T) (receipt ethtypes.Receipt) {
	return readMuseReceipt(t, "invalid_mrc20_withdraw_to_external")
}

// ValidMrc20WithdrawToETHReceipt is a receipt for a MRC20 withdrawal to an eth address
func ValidMrc20WithdrawToETHReceipt(t *testing.T) (receipt ethtypes.Receipt) {
	return readMuseReceipt(t, "mrc20_withdraw_to_eth")
}

// ValidMRC20WithdrawToBTCReceipt is a receipt for a MRC20 withdrawal to a BTC address
// receiver is bc1qysd4sp9q8my59ul9wsf5rvs9p387hf8vfwatzu
func ValidMRC20WithdrawToBTCReceipt(t *testing.T) (receipt ethtypes.Receipt) {
	return readMuseReceipt(t, "mrc20_withdraw_to_btc")
}

// ValidMuseSentDestinationExternalReceipt is a receipt for a Muse sent to an external destination
func ValidMuseSentDestinationExternalReceipt(t *testing.T) (receipt ethtypes.Receipt) {
	return readMuseReceipt(t, "muse_sent_destination_external")
}
...
func ValidGatewayWithdrawToSOLChainReceipt(t *testing.T) (receipt ethtypes.Receipt) {
	return readMuseReceipt(t, "gateway_withdraw_to_sol")
}
...
func InvalidGatewayWithdrawToSOLChainReceipt(t *testing.T) (receipt ethtypes.Receipt) {
	return readMuseReceipt(t, "invalid_gateway_withdraw_to_sol")
}
...
func ValidGatewayWithdrawAndCallToSOLChainReceipt(t *testing.T) (receipt ethtypes.Receipt) {
	return readMuseReceipt(t, "gateway_withdraw_and_call_to_sol")
}
...
func ValidGatewayCallToSOLChainReceipt(t *testing.T) (receipt ethtypes.Receipt) {
	return readMuseReceipt(t, "gateway_call_to_sol")
}
...
func InvalidGatewayCallToSOLChainReceipt(t *testing.T) (receipt ethtypes.Receipt) {
	return readMuseReceipt(t, "invalid_gateway_call_to_sol")
}
