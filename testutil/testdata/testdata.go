...
const (
	...
	MuseReceiptPrefixPath = "muse/receipt_"

	TxPrefixPath = "ethereum/tx_"
	TxsCount     = 81
)

// TypesFiles contains the embedded files of different types in MuseChain
//
//go:embed types/*
...
// https://github.com/RWAs-labs/muse/issues/1874
...
// ReadMuseReceipt reads a receipt from a file by filename.
// TODO: centralize test data
// https://github.com/RWAs-labs/muse/issues/1874
func ReadMuseReceipt(name string) (receipt types.Receipt, err error) {
	filePath := fmt.Sprintf("%s%s.json", MuseReceiptPrefixPath, name)
	return readReceipt(filePath, museFiles)
}
