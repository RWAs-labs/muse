...
# The script run the musee2e CLI to run local end-to-end tests
...
# upgrade: run the local e2e once, then restart museclientd at upgrade height and run the local e2e again
...
# Copy the operator keys from the musecore nodes to the orchestrator
copy_operator_keys() {
  local nodes=("musecore0" "musecore1" "musecore2" "musecore3" "musecore-new-validator")

  for node in "${nodes[@]}"; do
    # Skip if key not available
    ssh -q root@$node "exit" 2>/dev/null || continue
    ssh root@$node "test -d /root/.musecored/keyring-test/" || continue

    mkdir -p /root/$node/
    scp -r root@$node:/root/.musecored/keyring-test/ /root/$node/keyring-test/ || continue
    ...
    musecored keys rename operator operator$node_id --home=/root/$node/ --keyring-backend=test --yes
    cp -r /root/$node/keyring-test/* /root/.musecored/keyring-test/
  done

  echo "Key copying process completed"
}

get_musecored_version() {
  retries=10
  node_info=""
  for ((attempt=1; attempt<=$retries; attempt++)); do
    node_info=$(curl -s -f musecore0:1317/cosmos/base/tendermint/v1beta1/node_info)
    if [[ $? == 0 ]]; then
  ...
  echo "Unable to get musecored version after ${retries} retries"
  exit 1
}
...
# unlock legacy muse tester accounts
fund_eth_from_config '.additional_accounts.user_legacy_muse.evm_address' 10000 "muse tester"

# unlock legacy mevm message passing tester accounts
fund_eth_from_config '.additional_accounts.user_legacy_mevm_mp.evm_address' 10000 "mevm mp tester"
...
mkdir -p /root/.musecored/keyring-test/
...
### Run musee2e command depending on the option passed
...
if [ "$LOCALNET_MODE" == "tss-migrate" ]; then
  if [[ ! -f "$deployed_config_path" ]]; then
    ...
    musee2e local $E2E_ARGS --setup-only --config config.yml --config-out "$deployed_config_path" --skip-header-proof
    if [ $? -ne 0 ]; then
  ...
  musee2e local $E2E_ARGS --skip-setup --config "$deployed_config_path"  --skip-header-proof --test-tss-migration
  if [ $? -ne 0 ]; then
  ...
  musee2e local $E2E_ARGS --skip-setup --config "$deployed_config_path" \
    --skip-bitcoin-setup --light --skip-header-proof --skip-precompiles
  MUSEE2E_EXIT_CODE=$?
  if [ $MUSEE2E_EXIT_CODE -eq 0 ]; then
...
if [ "$LOCALNET_MODE" == "upgrade" ]; then
  ...
  # Run the e2e tests, then restart museclientd at upgrade height and run the e2e tests again

  # set upgrade height to 225 by default
  UPGRADE_HEIGHT=${UPGRADE_HEIGHT:=225}
  OLD_VERSION=$(get_musecored_version)
  COMMON_ARGS="--skip-header-proof --skip-tracker-check"

  if [[ ! -f "$deployed_config_path"  ]]; then
    [[ -n $CI ]] && echo "::group::setup"
    musee2e local $E2E_ARGS --setup-only --config config.yml --config-out "$deployed_config_path"  ${COMMON_ARGS}
    if [ $? -ne 0 ]; then
      echo "e2e setup failed"
      exit 1
    fi
  ...
  # Run musee2e, if the upgrade height is greater than 100 to populate the state
  if [ "$UPGRADE_HEIGHT" -gt 100 ]; then
    ...
    musee2e local $E2E_ARGS --skip-setup --config "$deployed_config_path" --light --skip-precompiles ${COMMON_ARGS}
    if [ $? -ne 0 ]; then
      echo "first e2e failed"
      exit 1
    fi
  ...
  NEW_VERSION=$(get_musecored_version)
  ...
  # wait for mevm endpoint to come up
  sleep 10
  ...
  if [ "$UPGRADE_HEIGHT" -lt 100 ]; then
    musee2e local $E2E_ARGS --skip-setup --config "$deployed_config_path" --light ${COMMON_ARGS}
  else
    musee2e local $E2E_ARGS --skip-setup --config "$deployed_config_path" --skip-bitcoin-setup --light ${COMMON_ARGS}
  fi
  ...
  if [[ ! -f "$deployed_config_path"  ]]; then
    ...
    musee2e local $E2E_ARGS --config config.yml --setup-only --config-out "$deployed_config_path"
    if [ $? -ne 0 ]; then
      echo "e2e setup failed"
      exit 1
    fi
