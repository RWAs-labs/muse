...
func Test_Methods(t *testing.T) {
	t.Run("should fail when trying to run deposit as read only method", func(t *testing.T) {
		...
		ts.mockVMContract.Input = packInputArgs(
			t,
			methodID,
			[]interface{}{ts.mrc20Address, big.NewInt(0)}...,
		)
	...
	t.Run("should fail when bank has 0 token allowance", func(t *testing.T) {
		ts := setupChain(t)
		caller := fungibletypes.ModuleAddressEVM
		ts.fungibleKeeper.DepositMRC20(ts.ctx, ts.mrc20Address, caller, big.NewInt(1000))
		...
		ts.mockVMContract.Input = packInputArgs(
			t,
			methodID,
			[]interface{}{ts.mrc20Address, big.NewInt(1000)}...,
		)
	...
	t.Run("should fail when trying to deposit 0", func(t *testing.T) {
		...
		// Allow bank to spend 500 MRC20 tokens.
	...
	t.Run("should fail when trying to deposit more than allowed to bank", func(t *testing.T) {
		...
		ts.mockVMContract.Input = packInputArgs(
			t,
			methodID,
			[]interface{}{ts.mrc20Address, big.NewInt(501)}...,
		)
		...
		require.Contains(
			t,
			err.Error(),
			"unexpected error in LockMRC20InBank: failed allowance check: invalid allowance, got 500, wanted 501",
		)
		...
		ts.mockVMContract.Input = packInputArgs(
			t,
			methodID,
			[]interface{}{ts.mrc20Address, caller}...,
		)
	...
	t.Run("should fail when trying to deposit more than user balance", func(t *testing.T) {
		...
		ts.mockVMContract.Input = packInputArgs(
			t,
			methodID,
			[]interface{}{ts.mrc20Address, big.NewInt(1001)}...,
		)
	...
	t.Run("should deposit tokens and retrieve balance of cosmos coin", func(t *testing.T) {
		...
		ts.mockVMContract.Input = packInputArgs(
			t,
			methodID,
			[]interface{}{ts.mrc20Address, big.NewInt(500)}...,
		)
