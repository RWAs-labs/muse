...
import (
	...
	"github.com/RWAs-labs/muse/precompiles/types"
...
func Test_Bech32ToHexAddress(t *testing.T) {
	...
	t.Run("should succeed with valid bech32 address", func(t *testing.T) {
		args := []interface{}{"muse1h8duy2dltz9xz0qqhm5wvcnj02upy887fyn43u"}
	...
	t.Run("should fail if invalid bech32 address", func(t *testing.T) {
		...
		t.Run("invalid number of arguments", func(t *testing.T) {
			args := []interface{}{"muse1h8duy2dltz9xz0qqhm5wvcnj02upy887fyn43u", "second argument"}
			_, err := contract.Bech32ToHexAddr(&methodID, args)
...
func Test_Bech32ify(t *testing.T) {
	...
	t.Run("should succeed with muse HRP", func(t *testing.T) {
		args := []interface{}{"muse", common.HexToAddress("0xB9Dbc229Bf588A613C00BEE8e662727AB8121cfE")}

		rawBytes, err := contract.Bech32ify(&methodID, args)
		require.NoError(t, err, "Bech32ify prefix muse should not return an error")

		museAddr := string(rawBytes[64:107])
		require.Equal(
			t,
			"muse1h8duy2dltz9xz0qqhm5wvcnj02upy887fyn43u",
			museAddr,
			"Bech32ify prefix muse should return the correct address, got: %v",
			museAddr,
		)
	})
