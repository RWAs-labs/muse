...
For example: musee2e run deposit:1000 withdraw: --config config.yml`,
		RunE:         runE2ETest,
...
func runE2ETest(cmd *cobra.Command, args []string) error {
	...
	err = processMRC20Flags(cmd, &conf)
	if err != nil {
		return fmt.Errorf("process MRC20 flags: %w", err)
	}
	...
	// if keys are defined for all policy accounts, we initialize a MUSE tx server allowing to send admin actions
	...
	if emergencyKey != "" && operationalKey != "" && adminKey != "" {
		...
		runnerOpts = append(runnerOpts, runner.WithMuseTxServer(museTxServer))
	}
