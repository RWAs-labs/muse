...
func startBitcoinTests(
	...
	bitcoinWithdrawTests := []string{
		...
		e2etests.TestLegacyMuseWithdrawBTCRevertName,
	}
	bitcoinWithdrawTestsAdvanced := []string{
		...
		// https://github.com/RWAs-labs/muse/blob/5c2a8ffbc702130fd9538b1cd7640d0e04d3e4f6/museclient/chains/bitcoin/observer/outbound.go#L27
...
func bitcoinTestRoutines(
	...
	// initialize funds
	// send BTC to TSS for gas fees and to tester MEVM address
	if initNetwork {
		...
		// donate BTC to TSS and send BTC to MEVM addresses
...
func initBitcoinRunner(
	...
	if initNetwork {
		...
		// deposit ETH and ERC20 tokens on MuseChain
