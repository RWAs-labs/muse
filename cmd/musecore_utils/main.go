...
//const amount = "100000000" // Amount in amuse
...
func main() {
	file, err := filepath.Abs(filepath.Join("cmd", "musecore_utils", "address-list.json"))
	if err != nil {
	...
	fileS, err := filepath.Abs(filepath.Join("cmd", "musecore_utils", "successful_address.json"))
	if err != nil {
		fmt.Printf("error getting absolute path of successful_address.json: %s\n", err)
		os.Exit(1)
	}
	fileF, err := filepath.Abs(filepath.Join("cmd", "musecore_utils", "failed_address.json"))
	if err != nil {
	...
	args = append(
		...
		[]string{
			...
			"--gas=auto",
			"--gas-adjustment=2",
			"--gas-prices=0.001amuse",
			"--node",
			node,
		}...)

	// #nosec G204
	cmd := exec.Command("musecored", args...)
	output, err := cmd.CombinedOutput()
	if err != nil {
		fmt.Println(cmd.String())
