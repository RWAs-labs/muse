...
import (
	...
	musetoolcontext "github.com/RWAs-labs/muse/cmd/musetool/context"
)
...
func TrackCCTX(cmd *cobra.Command, args []string) error {
	...
	for i := 0; i < maxCCTXChainLength; i++ {
		...
		ctx, err := musetoolcontext.NewContext(context.Background(), chainID, chainHash, configFile)
		if err != nil {
...
func trackCCTX(ctx *musetoolcontext.Context) (*cctx.TrackingDetails, error) {
	var (
	...
	// Update cctx status from musecore.This copies the status from musecore to the cctx details.The cctx status can only be `PendingInboundVoting` or `PendingInboundConfirmation` at this point
	...
	// The cctx details now have status from musecore, we have not tried to a get more granular status from the outbound chain yet.
