...
import (
	...
	"github.com/RWAs-labs/muse/pkg/parsers"
...
func AddObserverListCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Add a list of observers to the observer mapper ,default path is ~/.musecored/os_info/observer_info.json",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, info := range observerInfo {
				...
				if info.MuseClientGranteeAddress == "" || info.ObserverAddress == "" {
					panic("MuseClientGranteeAddress or ObserverAddress is empty")
				}
				...
				if info.MuseClientGranteePubKey != "" {
					pubkey, err := crypto.NewPubKey(info.MuseClientGranteePubKey)
					if err != nil {
						panic(err)
					}
					...
					na := types.NodeAccount{
						...
						GranteeAddress: info.MuseClientGranteeAddress,
				...
				balances = append(balances, banktypes.Balance{
					Address: info.MuseClientGranteeAddress,
					Coins:   HotkeyBalance,
				})
				keygenPubKeys = append(keygenPubKeys, info.MuseClientGranteePubKey)
			}
			...
			museCrossChainGenState := crosschaintypes.GetGenesisStateFromAppState(cdc, appState)
			tss := types.TSS{}
			if keyGenBlock == 0 {
			...
			museObserverGenState := types.GetGenesisStateFromAppState(cdc, appState)
			museObserverGenState.Observers = observerSet
			museObserverGenState.NodeAccountList = nodeAccounts
			museObserverGenState.Tss = &tss
			keyGenStatus := types.KeygenStatus_PendingKeygen
			if keyGenBlock == 0 {
				keyGenStatus = types.KeygenStatus_KeyGenSuccess
			}
			museObserverGenState.Keygen = &types.Keygen{
				Status:         keyGenStatus,
			...
			museCrossChainStateBz, err := json.Marshal(museCrossChainGenState)
			if err != nil {
				return fmt.Errorf("failed to marshal Observer List into Genesis File: %w", err)
			}

			museObserverStateBz, err := json.Marshal(museObserverGenState)
			if err != nil {
				return fmt.Errorf("failed to marshal Observer List into Genesis File: %w", err)
			}
			...
			appState[types.ModuleName] = museObserverStateBz
			appState[authz.ModuleName] = authZStateBz
			appState[crosschaintypes.ModuleName] = museCrossChainStateBz
...
func generateGrants(info parsers.ObserverInfoReader) []authz.GrantAuthorization {
	...
	if info.MuseClientGranteeAddress != "" {
		sdk.MustAccAddressFromBech32(info.MuseClientGranteeAddress)
		grants = append(grants, addMuseClientGrants(grants, info)...)
	}
...
func addMuseClientGrants(
	...
	for _, txType := range txTypes {
		...
		grants = append(grants, authz.GrantAuthorization{
			...
			Grantee:       info.MuseClientGranteeAddress,
			Authorization: auth,
			Expiration:    nil,
		})
	}
