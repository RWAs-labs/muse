...
import (
	...
	musecored "github.com/RWAs-labs/muse/cmd/musecored"
...
func Test_ModifyCrossChainState(t *testing.T) {
	t.Run("successfully modify cross chain state to reduce data", func(t *testing.T) {
		...
		err := musecored.ModifyCrosschainState(appState, importData, cdc)
...
func Test_ModifyObserverState(t *testing.T) {
	t.Run("successfully modify observer state to reduce data", func(t *testing.T) {
		...
		appState := sample.AppState(t)
		importData := GetImportData(t, cdc, 100)
		err := musecored.ModifyObserverState(appState, importData, cdc)
		require.NoError(t, err)

		modifiedObserverAppState := observertypes.GetGenesisStateFromAppState(cdc, appState)
		require.Len(t, modifiedObserverAppState.Ballots, musecored.MaxItemsForList)
		require.Len(t, modifiedObserverAppState.NonceToCctx, musecored.MaxItemsForList)
	})
...
func Test_ImportDataIntoFile(t *testing.T) {
	t.Run("successfully import data into file and modify data", func(t *testing.T) {
		...
		err := musecored.ImportDataIntoFile(appGenesis, importAppGenesis, cdc, true)
		require.NoError(t, err)
		...
		require.Len(t, crosschainStateAfterImport.CrossChainTxs, musecored.MaxItemsForList)
		require.Len(t, crosschainStateAfterImport.InboundHashToCctxList, musecored.MaxItemsForList)
		require.Len(t, crosschainStateAfterImport.FinalizedInbounds, musecored.MaxItemsForList)
	...
	t.Run("successfully import data into file without modifying data", func(t *testing.T) {
		...
		err := musecored.ImportDataIntoFile(appGenesis, importAppGenesis, cdc, false)
		require.NoError(t, err)
