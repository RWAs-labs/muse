...
// Mevm specific fields. These fields are only available if the outbound or revert was involved in a MEVM transaction, as a deposit
...
// NewMEVMErrorMessage is s special case of NewCCTXErrorMessage which is called by MEVM specific code.
// This creates a CCTXErrorMessage with MEVM specific fields like Method, Contract, Args
func NewMEVMErrorMessage(
	method string,
...
func NewCCTXErrorJSONMessage(message string, newError error) string {
	...
	if newError != nil {
		...
		for _, e := range errorLists {
			...
			// the method, contract, args, revert_reason fields are present only in MEVM errors and are directly added to the CCTXErrorMessage struct
			default:
				{
