...
func TestParsing(t *testing.T) {
	...
	t.Run("Deposit", func(t *testing.T) {
		...
		const (
			expectedSender  = "0:9594c719ec4c95f66683b2fb1ca0b09de4a41f6fb087ba4c8d265b96a4cce50f"
			mevmRecipient   = "0xA1eb8D65b765D259E7520B791bc4783AdeFDd998"
		...
		assert.Equal(t, mevmRecipient, deposit.Recipient.Hex())
	...
	t.Run("Deposit and call", func(t *testing.T) {
		...
		assert.Equal(t, mevmRecipient, depositAndCall.Recipient.Hex())
	...
	t.Run("Deposit and call ABI", func(t *testing.T) {
		...
		const (
			...
			mevmRecipient   = "0x13ad4f89050E83e8F485BB8349b40d3b89833790"
			expectedDeposit = 94_800_000 // 0.1 TON - tx fee

			// cast abi-encode "fn(string)" "MUSE ON TON YEAH"
	...
	t.Run("Irrelevant tx", func(t *testing.T) {
		t.Run("Failed tx", func(t *testing.T) {
			...
			// 102 is 'unknown op'
			// https://github.com/RWAs-labs/protocol-contracts-ton/blob/main/contracts/common/errors.fc
...
func TestSnakeData(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "muse repeated 10 times", data: []byte(strings.Trim(strings.Repeat("MuseChain ", 10), " "))},
		{name: "muse repeated 50 times", data: []byte(strings.Trim(strings.Repeat("MuseChain ", 50), " "))},
