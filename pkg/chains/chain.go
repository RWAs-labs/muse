...
// IsMuseChain returns true if the chain is a MuseChain chain
func (chain Chain) IsMuseChain() bool {
	return chain.Network == Network_muse
}

// IsExternalChain returns true if the chain is an ExternalChain chain, not MuseChain
...
// IsMuseChain returns true if the chain is a Muse chain
// additionalChains is a list of additional chains to search from
// in practice, it is used in the protocol to dynamically support new chains without doing an upgrade
func IsMuseChain(chainID int64, additionalChains []Chain) bool {
	return ChainIDInChainList(chainID, ChainListByNetwork(Network_muse, additionalChains))
}
