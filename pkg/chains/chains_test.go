...
func TestMuseChainFromCosmosChainID(t *testing.T) {
	tests := []struct {
		name     string
		chainID  string
		expected chains.Chain
		...
		{
			name:     "MuseChainMainnet",
			chainID:  "cosmoshub_7000-1",
			expected: chains.MuseChainMainnet,
			wantErr:  false,
		},
		{
			name:     "MuseChainTestnet",
			chainID:  "cosmoshub_7001-1",
			expected: chains.MuseChainTestnet,
			wantErr:  false,
		},
		{
			name:     "MuseChainDevnet",
			chainID:  "cosmoshub_70000-1",
			expected: chains.MuseChainDevnet,
			wantErr:  false,
		},
		{
			name:     "MuseChainPrivnet",
			chainID:  "cosmoshub_101-1",
			expected: chains.MuseChainPrivnet,
			wantErr:  false,
		},
		{
			name:     "unknown chain",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := chains.MuseChainFromCosmosChainID(tt.chainID)
			if tt.wantErr {
				require.Error(t, err)
...
func TestMuseChainFromChainID(t *testing.T) {
	tests := []struct {
		name     string
		chainID  int64
		expected chains.Chain
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := chains.MuseChainFromChainID(tt.chainID)
			if tt.wantErr {
				require.Error(t, err)
				require.ErrorIs(t, err, chains.ErrNotMuseChain)
				require.Equal(t, chains.Chain{}, result)
			} else {
				require.NoError(t, err)
				require.Equal(t, tt.expected, result)
