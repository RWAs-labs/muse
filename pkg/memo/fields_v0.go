...
import (
	...
	musebits "github.com/RWAs-labs/muse/pkg/math/bits"
...
type FieldsV0 struct {
	// Receiver is the MEVM receiver address
	Receiver common.Address

	// Payload is the calldata passed to MEVM contract call
	Payload []byte
...
func (f *FieldsV0) Validate(opCode OpCode, dataFlags uint8) error {
	...
	if musebits.IsBitSet(dataFlags, bitPosReceiver) && crypto.IsEmptyAddress(f.Receiver) {
	...
	if musebits.IsBitSet(dataFlags, bitPosAbortAddress) && !common.IsHexAddress(f.RevertOptions.AbortAddress) {
...
func (f *FieldsV0) packFields(codec Codec, dataFlags uint8) ([]byte, error) {
	...
	if musebits.IsBitSet(dataFlags, bitPosAbortAddress) {
...
func (f *FieldsV0) unpackFields(codec Codec, dataFlags byte, data []byte) error {
	...
	// add 'revertMessage' argument optionally
	f.RevertOptions.CallOnRevert = musebits.IsBitSet(dataFlags, bitPosRevertMessage)
	if f.RevertOptions.CallOnRevert {
		codec.AddArguments(ArgRevertMessage(&f.RevertOptions.RevertMessage))
	}
