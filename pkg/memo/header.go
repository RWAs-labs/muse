...
func (h *Header) EncodeToBytes() ([]byte, error) {
	...
	var ctrlByte1 byte
	ctrlByte1 = musebits.SetBits(ctrlByte1, maskVersion, h.Version)
	ctrlByte1 = musebits.SetBits(ctrlByte1, maskEncodingFormat, byte(h.EncodingFmt))
	data[1] = ctrlByte1

	// set operation code, reserved bits
	var ctrlByte2 byte
	ctrlByte2 = musebits.SetBits(ctrlByte2, maskOpCode, byte(h.OpCode))
	ctrlByte2 = musebits.SetBits(ctrlByte2, maskCtrlReserved, h.Reserved)
	data[2] = ctrlByte2
...
func (h *Header) DecodeFromBytes(data []byte) error {
	...
	ctrlByte1 := data[1]
	h.Version = musebits.GetBits(ctrlByte1, maskVersion)
	h.EncodingFmt = EncodingFormat(musebits.GetBits(ctrlByte1, maskEncodingFormat))

	// extract operation code, reserved bits
	ctrlByte2 := data[2]
	h.OpCode = OpCode(musebits.GetBits(ctrlByte2, maskOpCode))
	h.Reserved = musebits.GetBits(ctrlByte2, maskCtrlReserved)
