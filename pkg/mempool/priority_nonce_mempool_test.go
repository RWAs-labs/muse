...
func (s *MempoolTestSuite) TestPriorityNonceTxOrder() {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("cosmos tx test: %s", tt.name), func(t *testing.T) {
			pool := musemempool.NewPriorityMempool()
			...
			require.NoError(t, musemempool.IsEmpty(pool))
		})
	}
...
func (s *MempoolTestSuite) TestIterator() {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("cosmos tx test: %s", tt.name), func(t *testing.T) {
			pool := musemempool.DefaultPriorityMempool()
...
func (s *MempoolTestSuite) TestPriorityTies() {
	...
	for i := 0; i < 100; i++ {
		s.mempool = musemempool.NewPriorityMempool()
		var shuffled []txSpec
		for _, t := range txSet {
			tx := txSpec{
				p: t.p,
...
func (s *MempoolTestSuite) TestRandomGeneratedTxs() {
	s.iterations = 0
	s.mempool = musemempool.NewPriorityMempool(musemempool.PriorityNonceWithOnRead(func(tx sdk.Tx) {
		s.iterations++
	}))
	t := s.T()
	ctx := sdk.NewContext(nil, cmtproto.Header{}, false, log.NewNopLogger())
...
func TestPriorityNonceMempool_NextSenderTx(t *testing.T) {
	...
	mp := musemempool.NewPriorityMempool()
...
func TestNextSenderTx_TxLimit(t *testing.T) {
	...
	// unlimited
	mp := musemempool.NewPriorityMempool(musemempool.PriorityNonceWithMaxTx(0))
	for i, tx := range txs {
		c := ctx.WithPriority(tx.priority)
		require.NoError(t, mp.Insert(c, tx))
		require.Equal(t, i+1, mp.CountTx())
	...
	// limit: 3
	mp = musemempool.NewPriorityMempool(musemempool.PriorityNonceWithMaxTx(3))
	for i, tx := range txs {
		c := ctx.WithPriority(tx.priority)
		err := mp.Insert(c, tx)
		if i < 3 {
	...
	// disabled
	mp = musemempool.NewPriorityMempool(musemempool.PriorityNonceWithMaxTx(-1))
	for _, tx := range txs {
		c := ctx.WithPriority(tx.priority)
		err := mp.Insert(c, tx)
		require.NoError(t, err)
...
func TestNextSenderTx_TxReplacement(t *testing.T) {
	...
	feeBump := 20
	mp = musemempool.NewPriorityMempool(
		musemempool.PriorityNonceWithTxReplacement(func(op, np int64, oTx, nTx sdk.Tx) bool {
